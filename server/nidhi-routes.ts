...
// ── Atomic member number counter ─────────────────────────────────────────────
async function nextMemberNumber(tenantId: number): Promise<string> {
  const r = await db.execute(sql`
    INSERT INTO nidhi_member_counters (tenant_id, last_number)
    VALUES (${tenantId}, 1)
    ON CONFLICT (tenant_id) DO UPDATE
      SET last_number = nidhi_member_counters.last_number + 1
    RETURNING last_number`);
  return `MEM-${String((r.rows[0] as any).last_number).padStart(5, '0')}`;
}

// ── Nidhi nightly jobs (called from cron) ────────────────────────────────────
export async function runNidhiNightlyJobs() {
  try {
    // 1. Auto-mark NPA: loans overdue by 90+ days
    await db.execute(sql`
      UPDATE nidhi_loans
      SET status = 'npa', npa_date = CURRENT_DATE,
          npa_reason = 'Auto-marked: overdue > 90 days', updated_at = NOW()
      WHERE status = 'active' AND record_status = 1
        AND next_emi_date IS NOT NULL
        AND next_emi_date < CURRENT_DATE - INTERVAL '90 days'`);

    // 2. Auto-close matured FDs/RDs
    const matured = await db.execute(sql`
      SELECT id, tenant_id, member_id, current_balance, deposit_type, account_number
      FROM nidhi_deposits
      WHERE status = 'active' AND record_status = 1
        AND maturity_date IS NOT NULL AND maturity_date <= CURRENT_DATE`);
    for (const d of matured.rows as any[]) {
      await db.execute(sql`
        UPDATE nidhi_deposits SET status = 'matured', updated_at = NOW()
        WHERE id = ${d.id}`);
      await db.execute(sql`
        INSERT INTO nidhi_deposit_transactions
          (tenant_id, deposit_id, member_id, transaction_type, amount, balance_after, narration)
        VALUES (${d.tenant_id}, ${d.id}, ${d.member_id}, 'maturity',
                ${d.current_balance}, 0, ${'Auto-matured: ' + d.account_number})`);
    }

    // 3. Monthly interest accrual for all active loans across all nidhi tenants
    const activeLoans = await db.execute(sql`
      SELECT id, tenant_id, loan_number, outstanding_principal, interest_rate
      FROM nidhi_loans WHERE status = 'active' AND record_status = 1`);
    const today = new Date().toISOString().slice(0, 10);
    for (const loan of activeLoans.rows as any[]) {
      const monthlyInterest = Math.round(
        Number(loan.outstanding_principal) * (Number(loan.interest_rate) / 12 / 100) * 100);
      if (monthlyInterest <= 0) continue;
      createJournalWithLines(
        today,
        `Monthly interest accrual — Loan ${loan.loan_number}`,
        [
        ...
        { isAutoGenerated: true, sourceType: 'nidhi_interest_accrual', sourceId: String(loan.id), tenantId: loan.tenant_id }
      ).catch((e: any) => console.error('[NIDHI CRON] interest accrual:', e));
    }

    console.log(`[NIDHI CRON] Done — NPA check, ${matured.rows.length} FD/RD matured, ${activeLoans.rows.length} loans accrued`);

    // 4. Send WhatsApp reminders (EMI due, FD maturity, KYC pending)
    await sendNidhiWhatsAppReminders().catch((e: any) => console.error('[NIDHI CRON] WA reminders:', e.message));
  } catch (e: any) {
    console.error('[NIDHI CRON] Error:', e.message);
  }
}
...
router.post("/members", auth, async (req: any, res) => {
  ...
  const body = req.body;
  try {
    const memberNum = await nextMemberNumber(t);
...
// ── Dividend Declare + GL Posting ─────────────────────────────────────────────

async function ensureDividendTable() {
  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS nidhi_dividend_declarations (
      id              SERIAL PRIMARY KEY,
      tenant_id       INTEGER NOT NULL,
      financial_year  VARCHAR(10) NOT NULL,
      declaration_date DATE NOT NULL DEFAULT CURRENT_DATE,
      dividend_rate   NUMERIC(6,2) NOT NULL,
  ...
  `);
  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS nidhi_dividend_payouts (
      id              SERIAL PRIMARY KEY,
      tenant_id       INTEGER NOT NULL,
...
router.post("/dividend/declare", auth, async (req: any, res: any) => {
  const t = tid(req);
  const { rate, financial_year, declared_by, declaration_date } = req.body;
  try {
    if (!rate || !financial_year) return res.status(400).json({ message: 'rate and financial_year are required' });
    await ensureDividendTable();
    const dividendRate = Number(rate);
    const members = await db.execute(sql`
      SELECT id, name, member_number, shares_held, total_share_amount
      FROM nidhi_members WHERE tenant_id=${t} AND status='active' AND record_status=1`);
    const payouts = (members.rows as any[]).map(m => ({
      member_id: String(m.id),
      member_number: m.member_number,
      member_name: m.name,
      shares_held: Number(m.shares_held || 0),
      share_amount: Number(m.total_share_amount || 0),
      dividend_amount: Math.round(Number(m.total_share_amount) * (dividendRate / 100) * 100) / 100,
    }));
    const totalAmount = payouts.reduce((s, p) => s + p.dividend_amount, 0);
    const declDate = declaration_date || new Date().toISOString().slice(0, 10);

    const decl = await db.execute(sql`
      INSERT INTO nidhi_dividend_declarations (tenant_id, financial_year, declaration_date, dividend_rate, total_members, total_amount, declared_by)
      VALUES (${t}, ${financial_year}, ${declDate}, ${dividendRate}, ${payouts.length}, ${totalAmount}, ${declared_by||null})
      RETURNING *`);
    const declId = (decl.rows[0] as any).id;

    for (const p of payouts) {
      await db.execute(sql`
        INSERT INTO nidhi_dividend_payouts (tenant_id, declaration_id, member_id, member_number, member_name, shares_held, share_amount, dividend_amount)
        VALUES (${t}, ${declId}, ${p.member_id}, ${p.member_number}, ${p.member_name}, ${p.shares_held}, ${p.share_amount}, ${p.dividend_amount})`);
    }

    // GL posting: Dr Retained Earnings (3100), Cr Dividend Payable (2100)
    const totalPaise = Math.round(totalAmount * 100);
    if (totalPaise > 0) {
      createJournalWithLines(
        declDate,
        `Dividend declared FY ${financial_year} @ ${dividendRate}%`,
        [
          { accountCode: '3100', debit: totalPaise, credit: 0, memo: 'Retained earnings appropriated for dividend' },
          { accountCode: '2100', debit: 0, credit: totalPaise, memo: 'Dividend payable to members' },
        ],
        { isAutoGenerated: false, sourceType: 'nidhi_dividend', sourceId: String(declId), tenantId: t }
      ).then(() => db.execute(sql`UPDATE nidhi_dividend_declarations SET gl_posted=TRUE WHERE id=${declId}`))
       .catch((e: any) => console.error('[NIDHI DIVIDEND GL]', e.message));
    }

    res.status(201).json({
      declaration: decl.rows[0],
      total_members: payouts.length,
      total_dividend: totalAmount,
      gl_posted: totalPaise > 0,
      payouts_preview: payouts.slice(0, 5),
    });
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});

router.get("/dividend/history", auth, async (req: any, res: any) => {
  const t = tid(req);
  try {
    await ensureDividendTable();
    const rows = await db.execute(sql`
      SELECT * FROM nidhi_dividend_declarations WHERE tenant_id=${t} ORDER BY declaration_date DESC LIMIT 20`);
    res.json(rows.rows);
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});

router.get("/dividend/:id/payouts", auth, async (req: any, res: any) => {
  const t = tid(req);
  try {
    await ensureDividendTable();
    const rows = await db.execute(sql`
      SELECT * FROM nidhi_dividend_payouts WHERE tenant_id=${t} AND declaration_id=${req.params.id} ORDER BY member_number`);
    res.json(rows.rows);
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});

router.post("/dividend/:id/pay-member", auth, async (req: any, res: any) => {
  const t = tid(req);
  const { member_id, payment_mode } = req.body;
  try {
    await ensureDividendTable();
    const r = await db.execute(sql`
      UPDATE nidhi_dividend_payouts SET status='paid', paid_date=CURRENT_DATE, payment_mode=${payment_mode||'cash'}
      WHERE tenant_id=${t} AND declaration_id=${req.params.id} AND member_id=${member_id} AND status='pending'
      RETURNING *`);
    if (!r.rows.length) return res.status(404).json({ message: 'Payout record not found or already paid' });
    res.json(r.rows[0]);
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});
...
router.post("/loans", auth, async (req: any, res) => {
  ...
  const b = req.body;
  try {
    const loanCountR = await db.execute(sql`
      INSERT INTO nidhi_loan_counters (tenant_id, last_number)
      VALUES (${t}, 1)
      ON CONFLICT (tenant_id) DO UPDATE SET last_number = nidhi_loan_counters.last_number + 1
      RETURNING last_number`);
    const loanNum = `GL-${String((loanCountR.rows[0] as any).last_number).padStart(6,'0')}`;
    const principal = Number(b.principal_amount);
    const rate = Number(b.interest_rate);
    ...
    // KYC guard — member must have completed KYC
    const memberRow = await db.execute(sql`SELECT kyc_status, status, shares_held FROM nidhi_members WHERE id=${b.member_id} AND tenant_id=${t}`);
    if (!memberRow.rows.length) return res.status(404).json({ message: 'Member not found' });
    const mem = memberRow.rows[0] as any;
    if (mem.kyc_status === 'pending') return res.status(400).json({ message: 'KYC not completed. Complete member KYC before disbursing a loan.' });
    if (mem.status !== 'active') return res.status(400).json({ message: `Member is ${mem.status}. Only active members can receive loans.` });

    // NOF ratio guard — total deposits must not exceed 20× Net Owned Funds
    const nofRow = await db.execute(sql`SELECT COALESCE(SUM(total_share_amount),0) as nof FROM nidhi_members WHERE tenant_id=${t} AND status='active' AND record_status=1`);
    const depRow = await db.execute(sql`SELECT COALESCE(SUM(current_balance),0) as deposits FROM nidhi_deposits WHERE tenant_id=${t} AND status='active' AND record_status=1`);
    const nof = Number((nofRow.rows[0] as any).nof);
    const totalDeposits = Number((depRow.rows[0] as any).deposits);
    if (nof > 0 && totalDeposits > 0 && (totalDeposits / nof) > 20) {
      return res.status(400).json({ message: `Cannot disburse: deposit-to-NOF ratio ${(totalDeposits/nof).toFixed(1)}× exceeds RBI limit of 20×. Increase share capital before disbursing further loans.` });
    }
...
// Returns month-by-month trend for the last N months covering all key KPIs.

router.get("/compliance/trend", auth, async (req: any, res: any) => {
  const t = tid(req);
  const months = Math.min(Math.max(Number((req.query as any).months) || 12, 1), 24);
  try {
    // Monthly snapshot: new members, active deposits, active loan portfolio, NPA, NOF
    const trend = await db.execute(sql`
      WITH months AS (
        SELECT generate_series(
          date_trunc('month', NOW() - ((${months} - 1)::int || ' months')::interval),
          date_trunc('month', NOW()),
          '1 month'::interval
        ) AS month
      )
      SELECT
        to_char(mo.month, 'YYYY-MM') AS period,
        (SELECT COUNT(*) FROM nidhi_members
          WHERE tenant_id=${t} AND record_status=1
            AND created_at < mo.month + INTERVAL '1 month') AS cumulative_members,
        (SELECT COUNT(*) FROM nidhi_members
          WHERE tenant_id=${t} AND record_status=1
            AND DATE_TRUNC('month', created_at) = mo.month) AS new_members,
        (SELECT COALESCE(SUM(current_balance),0) FROM nidhi_deposits
          WHERE tenant_id=${t} AND status='active' AND record_status=1
            AND created_at < mo.month + INTERVAL '1 month') AS total_deposits,
        (SELECT COALESCE(SUM(outstanding_principal),0) FROM nidhi_loans
          WHERE tenant_id=${t} AND status='active' AND record_status=1
            AND created_at < mo.month + INTERVAL '1 month') AS total_loans,
        (SELECT COALESCE(SUM(outstanding_principal),0) FROM nidhi_loans
          WHERE tenant_id=${t} AND status='npa' AND record_status=1
            AND created_at < mo.month + INTERVAL '1 month') AS npa_amount,
        (SELECT COALESCE(SUM(total_share_amount),0) FROM nidhi_members
          WHERE tenant_id=${t} AND status='active' AND record_status=1
            AND created_at < mo.month + INTERVAL '1 month') AS net_owned_funds,
        (SELECT COUNT(*) FROM nidhi_members
          WHERE tenant_id=${t} AND kyc_status='approved' AND record_status=1
            AND created_at < mo.month + INTERVAL '1 month') AS kyc_approved
      FROM months mo
      ORDER BY mo.month`);

    const rows = (trend.rows as any[]).map(r => {
      const totalLoans = Number(r.total_loans);
      const npaAmount = Number(r.npa_amount);
      const nof = Number(r.net_owned_funds);
      const deposits = Number(r.total_deposits);
      const cumMembers = Number(r.cumulative_members);
      return {
        period: r.period,
        new_members: Number(r.new_members),
        cumulative_members: cumMembers,
        total_deposits: deposits,
        total_loans: totalLoans,
        npa_amount: npaAmount,
        npa_rate: totalLoans > 0 ? Math.round((npaAmount / totalLoans) * 10000) / 100 : 0,
        net_owned_funds: nof,
        deposit_to_nof_ratio: nof > 0 ? Math.round((deposits / nof) * 100) / 100 : 0,
        is_nof_compliant: nof === 0 || (deposits / nof) <= 20,
        kyc_approved: Number(r.kyc_approved),
        kyc_completion_rate: cumMembers > 0 ? Math.round((Number(r.kyc_approved) / cumMembers) * 10000) / 100 : 0,
      };
    });

    res.json({ months, trend: rows });
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});
...
// ── NDH-4 Annual Return (financial summary for RoC filing) ───────────────────
router.get("/rbi-returns/ndh4/:year", auth, async (req: any, res: any) => {
  const t = tid(req);
  const year = req.params.year;
  try {
    const [members, deposits, loans, shares, npa] = await Promise.all([
      db.execute(sql`SELECT COUNT(*) as total, SUM(CASE WHEN status='active' THEN 1 ELSE 0 END) as active FROM nidhi_members WHERE tenant_id=${t} AND record_status=1`),
      db.execute(sql`
        SELECT
          COALESCE(SUM(CASE WHEN deposit_type='savings' THEN current_balance ELSE 0 END),0) as savings,
          COALESCE(SUM(CASE WHEN deposit_type='fd' THEN current_balance ELSE 0 END),0) as fd,
          COALESCE(SUM(CASE WHEN deposit_type='rd' THEN current_balance ELSE 0 END),0) as rd,
          COALESCE(SUM(current_balance),0) as total
        FROM nidhi_deposits WHERE tenant_id=${t} AND status='active' AND record_status=1`),
      db.execute(sql`
        SELECT COUNT(*) as total_loans, COALESCE(SUM(principal_amount),0) as disbursed,
          COALESCE(SUM(outstanding_principal),0) as outstanding
        FROM nidhi_loans WHERE tenant_id=${t} AND record_status=1`),
      db.execute(sql`SELECT COALESCE(SUM(total_share_amount),0) as nof FROM nidhi_members WHERE tenant_id=${t} AND status='active' AND record_status=1`),
      db.execute(sql`SELECT COUNT(*) as npa_count, COALESCE(SUM(outstanding_principal),0) as npa_amount FROM nidhi_loans WHERE tenant_id=${t} AND status='npa' AND record_status=1`),
    ]);
    const m = members.rows[0] as any;
    const d = deposits.rows[0] as any;
    const l = loans.rows[0] as any;
    const s = shares.rows[0] as any;
    const n = npa.rows[0] as any;
    const nof = Number(s.nof);
    const totalDep = Number(d.total);
    const ratio = nof > 0 ? (totalDep / nof) : 0;

    const xml = `<?xml version="1.0" encoding="UTF-8"?>
<!-- NDH-4 Annual Return — Nidhi Company -->
<!-- Financial Year: ${year}-${Number(year)+1} -->
<!-- Generated: ${new Date().toISOString()} -->
<NDH4AnnualReturn>
  <FinancialYear>${year}-${Number(year)+1}</FinancialYear>
  <Membership>
    <TotalMembers>${m.total}</TotalMembers>
    <ActiveMembers>${m.active}</ActiveMembers>
  </Membership>
  <Deposits>
    <SavingsDeposits>${d.savings}</SavingsDeposits>
    <FixedDeposits>${d.fd}</FixedDeposits>
    <RecurringDeposits>${d.rd}</RecurringDeposits>
    <TotalDeposits>${d.total}</TotalDeposits>
  </Deposits>
  <Loans>
    <TotalLoansGranted>${l.total_loans}</TotalLoansGranted>
    <TotalDisbursed>${l.disbursed}</TotalDisbursed>
    <TotalOutstanding>${l.outstanding}</TotalOutstanding>
    <NPACount>${n.npa_count}</NPACount>
    <NPAAmount>${n.npa_amount}</NPAAmount>
  </Loans>
  <NetOwnedFunds>${nof}</NetOwnedFunds>
  <DepositToNOFRatio>${ratio.toFixed(2)}</DepositToNOFRatio>
  <RBICompliant>${ratio <= 20 ? 'YES' : 'NO'}</RBICompliant>
</NDH4AnnualReturn>`;

    res.setHeader('Content-Type', 'application/xml');
    res.setHeader('Content-Disposition', `attachment; filename="NDH4-${year}.xml"`);
    res.send(xml);
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});

// ── NDH-9 Self-declaration by directors ──────────────────────────────────────
router.post("/rbi-returns/ndh9", auth, async (req: any, res: any) => {
  const t = tid(req);
  const { financial_year, director_name, director_din, declaration_date } = req.body;
  try {
    const [members, deposits, loans, shares] = await Promise.all([
      db.execute(sql`SELECT COUNT(*) as total FROM nidhi_members WHERE tenant_id=${t} AND record_status=1`),
      db.execute(sql`SELECT COALESCE(SUM(current_balance),0) as total FROM nidhi_deposits WHERE tenant_id=${t} AND status='active' AND record_status=1`),
      db.execute(sql`SELECT COALESCE(SUM(outstanding_principal),0) as outstanding FROM nidhi_loans WHERE tenant_id=${t} AND status='active' AND record_status=1`),
      db.execute(sql`SELECT COALESCE(SUM(total_share_amount),0) as nof FROM nidhi_members WHERE tenant_id=${t} AND status='active' AND record_status=1`),
    ]);
    const totalMembers = Number((members.rows[0] as any).total);
    const totalDeposits = Number((deposits.rows[0] as any).total);
    const totalLoans = Number((loans.rows[0] as any).outstanding);
    const nof = Number((shares.rows[0] as any).nof);
    const ratio = nof > 0 ? totalDeposits / nof : 0;
    const isCompliant = ratio <= 20 && totalMembers >= 200;

    await ensureRBISubmissionsTable();
    await db.execute(sql`
      INSERT INTO nidhi_rbi_submissions (tenant_id, return_type, year, period, submitted_by)
      VALUES (${t}, 'NDH-9', ${parseInt(String(financial_year || new Date().getFullYear()))}, ${financial_year||null}, ${director_name||req.user?.username})`);
    ...
    res.json({
      ...
      director_name,
      director_din,
      declaration_date: declaration_date || new Date().toISOString().slice(0,10),
      data: { total_members: totalMembers, total_deposits: totalDeposits, total_loans: totalLoans, net_owned_funds: nof, deposit_nof_ratio: ratio.toFixed(2) },
      is_compliant: isCompliant,
      declaration: `I, ${director_name||'Director'} (DIN: ${director_din||'N/A'}), hereby declare that the information furnished in this return is true and correct to the best of my knowledge and belief.`,
      message: isCompliant ? 'Company is RBI-compliant. NDH-9 recorded.' : 'WARNING: Company may not meet RBI compliance thresholds (200 members or 20× NOF ratio).',
    });
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});

// ── Manual trigger for nightly jobs (admin/owner only) ───────────────────────
router.post("/admin/run-nightly-jobs", auth, async (req: any, res: any) => {
  try {
    await runNidhiNightlyJobs();
    res.json({ success: true, message: 'Nightly jobs completed' });
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});

// ── Feature 1: KYC Document Upload + Review Workflow ─────────────────────────

async function ensureKycDocsTable() {
  await db.execute(sql`
...
router.post("/members/:id/kyc-documents", auth, async (req: any, res: any) => {
  const t = tid(req);
  const { doc_type, file_name, file_data, mime_type } = req.body;
  try {
    if (!doc_type) return res.status(400).json({ message: 'doc_type is required (aadhar_front, aadhar_back, pan, photo, address_proof)' });
    await ensureKycDocsTable();
    const member = await db.execute(sql`SELECT id FROM nidhi_members WHERE id=${req.params.id} AND tenant_id=${t}`);
    if (!member.rows.length) return res.status(404).json({ message: 'Member not found' });
    const r = await db.execute(sql`
      INSERT INTO nidhi_kyc_documents (tenant_id, member_id, doc_type, file_name, file_data, mime_type, uploaded_by, status)
      VALUES (${t}, ${req.params.id}, ${doc_type}, ${file_name||null}, ${file_data||null}, ${mime_type||null}, ${req.user?.username||null}, 'pending')
      RETURNING id, doc_type, file_name, status, created_at`);
    res.status(201).json(r.rows[0]);
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});

router.get("/members/:id/kyc-documents", auth, async (req: any, res: any) => {
  const t = tid(req);
  try {
    await ensureKycDocsTable();
    const rows = await db.execute(sql`
      SELECT id, doc_type, file_name, mime_type, status, uploaded_by, reviewed_by, reviewed_at, rejection_reason, created_at
      FROM nidhi_kyc_documents WHERE tenant_id=${t} AND member_id=${req.params.id} ORDER BY created_at DESC`);
    res.json(rows.rows);
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});

router.put("/members/:id/kyc-approve", auth, async (req: any, res: any) => {
  const t = tid(req);
  try {
    await ensureKycDocsTable();
    await db.execute(sql`
      UPDATE nidhi_kyc_documents SET status='approved', reviewed_by=${req.user?.username||null}, reviewed_at=NOW()
      WHERE tenant_id=${t} AND member_id=${req.params.id} AND status='pending'`);
    const r = await db.execute(sql`
      UPDATE nidhi_members SET kyc_status='approved', updated_at=NOW()
      WHERE id=${req.params.id} AND tenant_id=${t} RETURNING id, member_number, name, kyc_status`);
    if (!r.rows.length) return res.status(404).json({ message: 'Member not found' });
    res.json({ ...r.rows[0], message: 'KYC approved. Member can now receive loans.' });
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});

router.put("/members/:id/kyc-reject", auth, async (req: any, res: any) => {
  const t = tid(req);
  const { reason } = req.body;
  try {
    await ensureKycDocsTable();
    await db.execute(sql`
      UPDATE nidhi_kyc_documents SET status='rejected', reviewed_by=${req.user?.username||null}, reviewed_at=NOW(), rejection_reason=${reason||null}
      WHERE tenant_id=${t} AND member_id=${req.params.id} AND status='pending'`);
    const r = await db.execute(sql`
      UPDATE nidhi_members SET kyc_status='rejected', updated_at=NOW()
      WHERE id=${req.params.id} AND tenant_id=${t} RETURNING id, member_number, name, kyc_status`);
    if (!r.rows.length) return res.status(404).json({ message: 'Member not found' });
    res.json({ ...r.rows[0], rejection_reason: reason, message: 'KYC rejected. Member must resubmit documents.' });
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});

// ── Feature 2: Penal Interest Auto-Calculation ────────────────────────────────

const PENAL_RATE_PER_ANNUM = 2; // 2% p.a. over normal rate on overdue EMIs

router.get("/loans/:id/penal-interest-quote", auth, async (req: any, res: any) => {
  const t = tid(req);
  try {
    const loan = await db.execute(sql`SELECT * FROM nidhi_loans WHERE id=${req.params.id} AND tenant_id=${t}`);
    if (!loan.rows.length) return res.status(404).json({ message: 'Loan not found' });
    ...
    const nextEmi = l.next_emi_date ? new Date(l.next_emi_date) : null;
    const daysOverdue = nextEmi && nextEmi < today ? Math.floor((today.getTime() - nextEmi.getTime()) / 86400000) : 0;
    const outstanding = Number(l.outstanding_principal);
    const penalInterest = daysOverdue > 0
      ? Math.round(outstanding * (PENAL_RATE_PER_ANNUM / 100) * (daysOverdue / 365) * 100) / 100
      : 0;
    res.json({
      ...
      penal_rate_per_annum: PENAL_RATE_PER_ANNUM,
      penal_interest_due: penalInterest,
      normal_emi: Number(l.emi_amount || 0),
      total_due: Math.round((Number(l.emi_amount || 0) + penalInterest) * 100) / 100,
    });
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});

// ── Feature 3: Loan Foreclosure Quote ────────────────────────────────────────

router.get("/loans/:id/foreclosure-quote", auth, async (req: any, res: any) => {
  const t = tid(req);
  try {
    const loan = await db.execute(sql`SELECT * FROM nidhi_loans WHERE id=${req.params.id} AND tenant_id=${t}`);
    if (!loan.rows.length) return res.status(404).json({ message: 'Loan not found' });
    const l = loan.rows[0] as any;
    if (l.status === 'closed') return res.status(400).json({ message: 'Loan is already closed' });
    const outstanding = Number(l.outstanding_principal);
    const today = new Date();
    const nextEmi = l.next_emi_date ? new Date(l.next_emi_date) : null;
    const daysOverdue = nextEmi && nextEmi < today ? Math.floor((today.getTime() - nextEmi.getTime()) / 86400000) : 0;
    // Accrued interest since last EMI
    const daysSinceLastEmi = l.last_emi_date
      ? Math.floor((today.getTime() - new Date(l.last_emi_date).getTime()) / 86400000)
      : 0;
    const accruedInterest = Math.round(outstanding * (Number(l.interest_rate) / 100) * (daysSinceLastEmi / 365) * 100) / 100;
    const penalInterest = daysOverdue > 0
      ? Math.round(outstanding * (PENAL_RATE_PER_ANNUM / 100) * (daysOverdue / 365) * 100) / 100
      : 0;
    // Foreclosure penalty: 1% of outstanding if closed before 50% tenure
    const emisPaid = Number(l.emis_paid || 0);
    const totalEmis = Number(l.total_emis || 1);
    const foreclosurePenalty = emisPaid < totalEmis / 2
      ? Math.round(outstanding * 0.01 * 100) / 100
      : 0;
    const totalForeclosure = Math.round((outstanding + accruedInterest + penalInterest + foreclosurePenalty) * 100) / 100;
    res.json({
      loan_id: req.params.id,
      loan_number: l.loan_number,
      outstanding_principal: outstanding,
      ...
      foreclosure_penalty: foreclosurePenalty,
      foreclosure_penalty_note: emisPaid < totalEmis / 2 ? '1% penalty — loan closed before 50% tenure' : 'No foreclosure penalty',
      total_foreclosure_amount: totalForeclosure,
      quote_date: today.toISOString().slice(0, 10),
      valid_until: new Date(today.getTime() + 7 * 86400000).toISOString().slice(0, 10),
    });
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});
...
router.get("/members/:id/share-certificate-pdf", auth, async (req: any, res: any) => {
  const t = tid(req);
  try {
    const [memberRows, companyRows] = await Promise.all([
      db.execute(sql`SELECT * FROM nidhi_members WHERE id=${req.params.id} AND tenant_id=${t}`),
      db.execute(sql`SELECT * FROM nidhi_company_profile WHERE tenant_id=${t} LIMIT 1`).catch(() => ({ rows: [] })),
    ]);
    if (!memberRows.rows.length) return res.status(404).json({ message: 'Member not found' });
    const m = memberRows.rows[0] as any;
    const co = (companyRows.rows[0] || {}) as any;
    const certNo = `SC-${String(t).padStart(4,'0')}-${String(m.member_number || m.id).replace(/[^0-9]/g,'').padStart(6,'0')}`;
    const doc = new PDFDocument({ margin: 60, size: 'A4' });
    const buffers: Buffer[] = [];
    doc.on('data', (b: Buffer) => buffers.push(b));
    doc.on('end', () => {
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `attachment; filename=share-certificate-${req.params.id}.pdf`);
      res.send(Buffer.concat(buffers));
    });

    // Border
    doc.rect(30, 30, doc.page.width - 60, doc.page.height - 60).stroke('#1a3a6b');
    doc.rect(35, 35, doc.page.width - 70, doc.page.height - 70).stroke('#c0a060');

    // Header
    doc.fontSize(20).font('Helvetica-Bold').fillColor('#1a3a6b')
      .text(co.company_name || 'NIDHI LIMITED', { align: 'center' });
    doc.fontSize(11).font('Helvetica').fillColor('#444')
      .text(co.registered_address || 'Registered under Nidhi Rules 2014', { align: 'center' });
    doc.moveDown(0.5);
    doc.fontSize(16).font('Helvetica-Bold').fillColor('#c0a060')
      .text('SHARE CERTIFICATE', { align: 'center' });
    doc.moveDown(0.3);
    doc.fontSize(10).font('Helvetica').fillColor('#333')
      .text(`Certificate No: ${certNo}`, { align: 'right' });
    doc.text(`Date: ${new Date().toLocaleDateString('en-IN', { day:'2-digit', month:'long', year:'numeric' })}`, { align: 'right' });

    doc.moveDown();
    doc.moveTo(60, doc.y).lineTo(doc.page.width - 60, doc.y).stroke('#c0a060');
    doc.moveDown(0.5);

    const row = (label: string, value: string) => {
      doc.fontSize(11).font('Helvetica-Bold').fillColor('#1a3a6b').text(label + ': ', { continued: true });
      doc.font('Helvetica').fillColor('#222').text(value);
    };

    row('This is to Certify that', m.name || '');
    row("Member Number", m.member_number || String(m.id));
    row("Father / Spouse Name", m.father_name || m.nominee_name || 'N/A');
    row("Address", m.address || 'N/A');
    row("Number of Shares Held", String(m.shares_held || 1));
    row("Face Value per Share", `₹${Number(m.share_value || 10).toLocaleString('en-IN')}`);
    row("Total Share Amount", `₹${Number(m.total_share_amount || (m.shares_held||1)*(m.share_value||10)).toLocaleString('en-IN')}`);
    row("Share Class", 'Ordinary Shares');
    row("Membership Date", m.membership_date ? new Date(m.membership_date).toLocaleDateString('en-IN') : 'N/A');
    ...
    doc.fontSize(9).fillColor('#555').font('Helvetica')
      .text('This certificate is issued under the authority of the Board of Directors of the company and is subject to the Memorandum and Articles of Association.', { align: 'center' });

    doc.moveDown(2);
    const sigY = doc.y;
    doc.fontSize(10).font('Helvetica-Bold').fillColor('#1a3a6b');
    doc.text('________________________', 80, sigY);
    doc.text('________________________', doc.page.width - 220, sigY);
    doc.fontSize(9).font('Helvetica').fillColor('#444');
    doc.text('Authorized Signatory', 80, sigY + 14);
    doc.text('Director / Secretary', doc.page.width - 220, sigY + 14);

    doc.end();
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});

// ── Feature 5: Loan NOC Letter PDF ───────────────────────────────────────────

router.get("/loans/:id/noc-pdf", auth, async (req: any, res: any) => {
  const t = tid(req);
  try {
    const [loanRows, companyRows] = await Promise.all([
      db.execute(sql`SELECT l.*, m.name as member_name, m.member_number, m.address as member_address, m.phone as member_phone FROM nidhi_loans l JOIN nidhi_members m ON m.id=l.member_id WHERE l.id=${req.params.id} AND l.tenant_id=${t}`),
      db.execute(sql`SELECT * FROM nidhi_company_profile WHERE tenant_id=${t} LIMIT 1`).catch(() => ({ rows: [] })),
    ]);
    if (!loanRows.rows.length) return res.status(404).json({ message: 'Loan not found' });
    const l = loanRows.rows[0] as any;
    if (l.status !== 'closed') return res.status(400).json({ message: 'NOC can only be generated for fully repaid (closed) loans' });
    const co = (companyRows.rows[0] || {}) as any;
    const nocNo = `NOC-${l.loan_number}-${new Date().getFullYear()}`;
    ...
    doc.on('end', () => {
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `attachment; filename=noc-${req.params.id}.pdf`);
      res.send(Buffer.concat(buffers));
    });

    doc.fontSize(18).font('Helvetica-Bold').fillColor('#1a3a6b')
      .text(co.company_name || 'NIDHI LIMITED', { align: 'center' });
    ...
    doc.fontSize(11).font('Helvetica').fillColor('#333')
      .text('(Loan Closure Certificate)', { align: 'center' });
    doc.moveDown(0.5);
    doc.moveTo(60, doc.y).lineTo(doc.page.width - 60, doc.y).stroke('#1a3a6b');
    doc.moveDown(0.8);

    const today = new Date().toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' });
    doc.fontSize(11).font('Helvetica').fillColor('#222')
      .text(`NOC No: ${nocNo}`, { align: 'right' })
      .text(`Date: ${today}`, { align: 'right' });
    doc.moveDown(0.8);

    const toName = `To,\n${l.member_name}\nMember No: ${l.member_number}\n${l.member_address || ''}`;
    doc.fontSize(11).font('Helvetica').fillColor('#222').text(toName);
    doc.moveDown();

    doc.fontSize(11).font('Helvetica-Bold').text('Sub: No Objection Certificate for Loan Repayment');
    doc.moveDown(0.5);
    doc.fontSize(11).font('Helvetica').fillColor('#333')
      .text(`Dear ${l.member_name},\n\nThis is to certify that you had availed a loan from ${co.company_name || 'our company'} with the following details:`, { paragraphGap: 4 });
    doc.moveDown(0.5);

    const details = [
      ['Loan Number', l.loan_number],
      ['Loan Type', l.loan_type || 'N/A'],
      ['Principal Amount', `₹${Number(l.principal_amount).toLocaleString('en-IN')}`],
      ['Disbursement Date', l.disbursement_date ? new Date(l.disbursement_date).toLocaleDateString('en-IN') : 'N/A'],
      ['Tenure', `${l.tenure_months || l.total_emis || 'N/A'} months`],
      ['Closure Date', l.updated_at ? new Date(l.updated_at).toLocaleDateString('en-IN') : today],
    ];
    for (const [k, v] of details) {
      doc.fontSize(11).font('Helvetica-Bold').fillColor('#1a3a6b').text(`${k}: `, { continued: true });
      doc.font('Helvetica').fillColor('#222').text(String(v));
    }

    doc.moveDown();
    doc.fontSize(11).font('Helvetica').fillColor('#333')
      .text('We confirm that the above loan has been fully repaid and there is no outstanding balance. We have no objection to the release of any security or documents held against this loan.')
      .moveDown(0.5)
      .text('This certificate is issued for the purpose of record and future reference only.')
      .moveDown(2);

    doc.moveTo(60, doc.y).lineTo(doc.page.width - 60, doc.y).stroke('#ccc');
    doc.moveDown(0.5);
    doc.fontSize(9).fillColor('#888')
      .text('This is a system-generated document. For queries, contact your Nidhi branch.', { align: 'center' });

    doc.moveDown(2);
    const sy = doc.y;
    doc.fontSize(11).font('Helvetica-Bold').fillColor('#1a3a6b');
    doc.text('________________________', 80, sy);
    doc.text('Authorised Signatory', 80, sy + 14).font('Helvetica').fontSize(9).fillColor('#444');
    doc.text(co.company_name || 'Nidhi Limited', 80, sy + 26);

    doc.end();
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});

// ── Feature 6: WhatsApp Reminders wired into nightly cron ────────────────────
// runNidhiNightlyJobs already exported above; extend it with WhatsApp notifications.
// Standalone endpoint to manually trigger all Nidhi WhatsApp reminders across tenants:

router.post("/admin/send-whatsapp-reminders", auth, async (req: any, res: any) => {
  try {
    const results = await sendNidhiWhatsAppReminders();
    res.json({ success: true, ...results });
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});

export async function sendNidhiWhatsAppReminders() {
  let emiSent = 0, maturitySent = 0, kycSent = 0;
  try {
    await db.execute(sql`
      CREATE TABLE IF NOT EXISTS nidhi_reminder_log (
        id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL, member_id TEXT,
        reminder_type VARCHAR(50), message TEXT, status VARCHAR(30) DEFAULT 'sent', sent_at TIMESTAMPTZ DEFAULT NOW()
      )
    `);

    // EMI due in 3 days across all tenants
    const emiDue = await db.execute(sql`
      SELECT l.tenant_id, l.id as loan_id, l.loan_number, l.emi_amount, l.next_emi_date, l.member_id,
             m.name, m.phone, m.member_number
      FROM nidhi_loans l JOIN nidhi_members m ON m.id::text=l.member_id::text
      WHERE l.status='active' AND l.record_status=1
        AND l.next_emi_date BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '3 days'
        AND m.phone IS NOT NULL`);
    for (const r of emiDue.rows as any[]) {
      const msg = `Dear ${r.name}, your EMI of ₹${Number(r.emi_amount).toLocaleString('en-IN')} is due on ${r.next_emi_date}. Loan: ${r.loan_number}. Pay at your nearest Nidhi branch.`;
      const ok = await whatsappService.sendTextMessage({ to: r.phone, message: msg }).catch(() => false);
      await db.execute(sql`INSERT INTO nidhi_reminder_log (tenant_id, member_id, reminder_type, message, status) VALUES (${r.tenant_id}, ${String(r.member_id)}, 'emi_due', ${msg}, ${ok?'sent':'failed'})`);
      if (ok) emiSent++;
    }

    // FD/RD maturing in 7 days
    const maturing = await db.execute(sql`
      SELECT d.tenant_id, d.id, d.account_number, d.deposit_type, d.maturity_date, d.principal_amount, d.member_id,
             m.name, m.phone, m.member_number
      FROM nidhi_deposits d JOIN nidhi_members m ON m.id::text=d.member_id::text
      WHERE d.status='active' AND d.record_status=1
        AND d.maturity_date BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '7 days'
        AND m.phone IS NOT NULL`);
    for (const r of maturing.rows as any[]) {
      const type = r.deposit_type === 'fd' ? 'Fixed Deposit' : 'Recurring Deposit';
      const msg = `Dear ${r.name}, your ${type} (${r.account_number}) of ₹${Number(r.principal_amount).toLocaleString('en-IN')} matures on ${r.maturity_date}. Please visit your Nidhi branch to renew or withdraw.`;
      const ok = await whatsappService.sendTextMessage({ to: r.phone, message: msg }).catch(() => false);
      await db.execute(sql`INSERT INTO nidhi_reminder_log (tenant_id, member_id, reminder_type, message, status) VALUES (${r.tenant_id}, ${String(r.member_id)}, 'fd_maturity', ${msg}, ${ok?'sent':'failed'})`);
      if (ok) maturitySent++;
    }

    // KYC pending reminder (send once a week on Mondays — nightly cron runs daily, check day)
    if (new Date().getDay() === 1) {
      const kycPending = await db.execute(sql`
        SELECT tenant_id, id, name, phone, member_number FROM nidhi_members
        WHERE kyc_status='pending' AND status='active' AND record_status=1 AND phone IS NOT NULL`);
      for (const r of kycPending.rows as any[]) {
        const msg = `Dear ${r.name} (Member: ${r.member_number}), your KYC documents are pending. Please visit your Nidhi branch with Aadhaar, PAN, and photo to complete KYC and unlock loan eligibility.`;
        const ok = await whatsappService.sendTextMessage({ to: r.phone, message: msg }).catch(() => false);
        await db.execute(sql`INSERT INTO nidhi_reminder_log (tenant_id, member_id, reminder_type, message, status) VALUES (${r.tenant_id}, ${String(r.id)}, 'kyc_pending', ${msg}, ${ok?'sent':'failed'})`);
        if (ok) kycSent++;
      }
    }

    console.log(`[NIDHI WA] EMI reminders: ${emiSent}, Maturity alerts: ${maturitySent}, KYC nudges: ${kycSent}`);
  } catch (e: any) {
    console.error('[NIDHI WA] Error:', (e as any).message);
  }
  return { emi_reminders_sent: emiSent, maturity_alerts_sent: maturitySent, kyc_reminders_sent: kycSent };
}
