...
async function handleGetGRNs(req: any, res: any) {
  const rows = await db.execute(sql`
    SELECT g.*, v.name as vendor_name FROM goods_receipt_notes g
...
// Unified action endpoint (approve or reject based on `action` field)
...
router.get("/audit-log", requireAuth, async (req: any, res) => {
  ...
  // audit_logs table uses table_name and record_id; user_id is a varchar
...
router.post("/bulk/expenses/approve", requireAuth, async (req: any, res) => {
  const { ids } = req.body; // array of expense claim IDs
...
// Bulk record payment on invoices
...
// Bulk submit purchase requisitions
...
// Save custom field values for an entity (upsert)
router.post("/custom-field-values/:entityType/:entityId", requireAuth, async (req: any, res) => {
  const { entityType, entityId } = req.params;
  const { values } = req.body; // { [fieldDefId]: value }
  const results = [];
