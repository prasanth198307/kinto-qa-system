...
router.get("/projects/:id", requireAuth, async (req: any, res) => {
  try {
    ...
    const [boq, milestones, timesheets, invoices, pos] = await Promise.all([
      db.execute(sql`SELECT * FROM boq_items WHERE project_id=${req.params.id} AND tenant_id=${tid} AND record_status=1 ORDER BY id`),
      db.execute(sql`SELECT * FROM billing_milestones WHERE project_id=${req.params.id} AND tenant_id=${tid} AND record_status=1 ORDER BY due_date`),
      db.execute(sql`SELECT ts.*, e.first_name||' '||e.last_name AS employee_name
      ...
      db.execute(sql`SELECT id, invoice_number, invoice_date, total_amount, status FROM invoices WHERE project_id=${req.params.id} AND tenant_id=${tid} AND record_status=1 ORDER BY invoice_date DESC`),
      db.execute(sql`SELECT id, po_number, po_date, total_amount, status FROM purchase_orders WHERE project_id=${req.params.id} AND tenant_id=${tid} AND record_status!=0 ORDER BY po_date DESC`),
    ]);
...
router.get("/projects/:id/pnl", requireAuth, async (req: any, res) => {
  try {
    ...
    if (e.code === '42P01') return res.json({ revenue: 0, cost: 0, gross_profit: 0, total_hours: 0, boq_value: 0, actual_cost: 0 });
    res.status(500).json({ message: e.message });
  }
});
