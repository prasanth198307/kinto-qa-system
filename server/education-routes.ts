...
router.post("/fee-payments", requireAuth, async (req: any, res) => {
  try {
    ...
    // Additional journal entry for fee collection
    const studentRow = await db.execute(sql`SELECT name FROM students WHERE id=${student_id} AND tenant_id=${tenantId}`).catch(() => ({ rows: [] }));
    const studentName = (studentRow.rows[0] as any)?.name || `Student #${student_id}`;
    const feeType = for_month ? `Fee - ${for_month}` : "Fee Payment";
    createJournalWithLines(
      paid_date || new Date().toISOString().slice(0, 10),
      `Fee collection - ${studentName} - ${feeType}`,
      [
        { accountCode: "1002", debit: Math.round(netAmount * 100), credit: 0, memo: "Bank receipt" },
        { accountCode: "4020", debit: 0, credit: Math.round(netAmount * 100), memo: feeType },
      ],
      { isAutoGenerated: true, sourceType: "fee_payment", sourceId: String(payment.id) }
    ).catch((e: any) => console.error("GL fail", e));
...
router.get("/certificates", requireAuth, async (req: any, res) => {
  const t = tid(req);
  const { student_id, type, academic_year } = req.query;
  try {
    await db.execute(sql`CREATE TABLE IF NOT EXISTS education_certificates (
      id SERIAL PRIMARY KEY, tenant_id INT NOT NULL,
      certificate_no VARCHAR(50) NOT NULL UNIQUE,
      student_id INT NOT NULL, type VARCHAR(50) NOT NULL,
    ...
    let q = sql`SELECT c.*, s.name as student_name, s.roll_number, s.admission_no as student_admission_no, s.class_id, s.section FROM education_certificates c LEFT JOIN students s ON s.id=c.student_id WHERE c.tenant_id=${t} AND c.record_status=1`;
    if (student_id) q = sql`${q} AND c.student_id=${parseInt(student_id as string)}`;
    if (type) q = sql`${q} AND c.type=${type}`;
    if (academic_year) q = sql`${q} AND c.academic_year=${academic_year}`;
    q = sql`${q} ORDER BY c.created_at DESC LIMIT 100`;
    const rows = await db.execute(q);
    res.json(rows.rows);
  } catch(e: any) { res.status(500).json({ message: e.message }); }
});

router.post("/certificates", requireAuth, async (req: any, res) => {
  const t = tid(req);
  const { student_id, type, issue_date, academic_year, issued_by, reason, marks_data, class_name, section, remarks, admission_no } = req.body;
  try {
    const year = new Date().getFullYear();
    const count = await db.execute(sql`SELECT COUNT(*) as n FROM education_certificates WHERE tenant_id=${t}`);
    const seq = String(Number((count.rows[0] as any).n) + 1).padStart(4, '0');
    const typeCode = ({ TC: 'TC', Bonafide: 'BF', 'Marks Card': 'MC', Character: 'CC', Conduct: 'CN', Migration: 'MG', Scholarship: 'SC' } as Record<string,string>)[type as string] || 'CERT';
    const certNo = `${typeCode}-${year}-${seq}`;
    const r = await db.execute(sql`INSERT INTO education_certificates (tenant_id, certificate_no, student_id, type, issue_date, academic_year, issued_by, reason, marks_data, class_name, section, remarks, admission_no) VALUES (${t}, ${certNo}, ${student_id}, ${type}, ${issue_date||new Date().toISOString().slice(0,10)}, ${academic_year||null}, ${issued_by||null}, ${reason||null}, ${marks_data ? JSON.stringify(marks_data) : null}, ${class_name||null}, ${section||null}, ${remarks||null}, ${admission_no||null}) RETURNING *`);
    res.json(r.rows[0]);
  } catch(e: any) { res.status(500).json({ message: e.message }); }
});

router.get("/certificates/:id/pdf-data", requireAuth, async (req: any, res) => {
  const t = tid(req);
  try {
    const r = await db.execute(sql`
      SELECT c.*, s.name as student_name, s.roll_number, s.dob, s.gender, s.nationality,
        s.religion, s.caste, s.mother_tongue, s.father_name, s.mother_name,
        s.address, s.phone, s.email, s.admission_no as student_admission_no,
        s.class_id, s.section as student_section,
        ts.company_name as school_name, ts.address as school_address,
        ts.phone as school_phone, ts.gstin, ts.logo_url
      FROM education_certificates c
      LEFT JOIN students s ON s.id = c.student_id
      LEFT JOIN tenant_settings ts ON ts.tenant_id = c.tenant_id
      WHERE c.id = ${parseInt(req.params.id)} AND c.tenant_id = ${t}
    `);
    if (!r.rows[0]) return res.status(404).json({ message: 'Certificate not found' });
    res.json(r.rows[0]);
  } catch(e: any) { res.status(500).json({ message: e.message }); }
});

router.put("/certificates/:id/cancel", requireAuth, async (req: any, res) => {
  const t = tid(req);
  try {
    await db.execute(sql`UPDATE education_certificates SET status='cancelled', remarks=${req.body.reason||'Cancelled'} WHERE id=${parseInt(req.params.id)} AND tenant_id=${t}`);
    res.json({ success: true });
  } catch(e: any) { res.status(500).json({ message: e.message }); }
});
...
// Teacher payroll bridge — link education teachers to hr_employees for payroll
router.get("/teachers/payroll-status", requireAuth, async (req: any, res) => {
  const t = tid(req);
  const { month, year } = req.query;
  try {
    const rows = await db.execute(sql`
      SELECT te.id as teacher_id, te.name as teacher_name, te.employee_id,
        te.designation, te.department, te.salary_grade,
        he.id as hr_employee_id, he.first_name, he.last_name,
        he.basic_salary, he.employment_type,
        ps.net_pay, ps.gross_pay, ps.status as payslip_status, ps.payment_date,
        ps.month as payroll_month, ps.year as payroll_year
      FROM teachers te
      LEFT JOIN hr_employees he ON he.id = te.employee_id AND he.tenant_id = ${t}
      LEFT JOIN hr_payslips ps ON ps.employee_id = he.id AND ps.tenant_id = ${t}
        ${month ? sql`AND ps.month = ${parseInt(month as string)}` : sql``}
        ${year ? sql`AND ps.year = ${parseInt(year as string)}` : sql``}
      WHERE te.tenant_id = ${t} AND te.record_status = 1
      ORDER BY te.name
    `);
    const linked = rows.rows.filter((r: any) => (r as any).hr_employee_id).length;
    const unlinked = rows.rows.filter((r: any) => !(r as any).hr_employee_id).length;
    res.json({ teachers: rows.rows, linked, unlinked, total: rows.rows.length });
  } catch(e: any) { res.status(500).json({ message: e.message }); }
});

router.post("/teachers/:id/link-employee", requireAuth, async (req: any, res) => {
  const t = tid(req);
  const { employee_id } = req.body;
  try {
    let empId = employee_id;
    if (!empId) {
      const teacher = await db.execute(sql`SELECT * FROM teachers WHERE id=${parseInt(req.params.id)} AND tenant_id=${t}`);
      if (!teacher.rows[0]) return res.status(404).json({ message: 'Teacher not found' });
      const te = teacher.rows[0] as any;
      const nameParts = (te.name || '').split(' ');
      const emp = await db.execute(sql`INSERT INTO hr_employees (tenant_id, employee_code, first_name, last_name, email, phone, designation, department, employment_type, date_of_joining, basic_salary, record_status)
        VALUES (${t}, ${'EMP-TCH-'+te.id}, ${nameParts[0]||te.name}, ${nameParts.slice(1).join(' ')||''}, ${te.email||null}, ${te.phone||null}, ${te.designation||'Teacher'}, ${'Academic'}, ${'full_time'}, ${te.joining_date||new Date().toISOString().slice(0,10)}, ${te.salary||0}, 1)
        RETURNING id`);
      empId = (emp.rows[0] as any).id;
    }
    await db.execute(sql`UPDATE teachers SET employee_id=${empId} WHERE id=${parseInt(req.params.id)} AND tenant_id=${t}`);
    res.json({ success: true, employee_id: empId });
  } catch(e: any) { res.status(500).json({ message: e.message }); }
});

router.get("/teachers/unlinked", requireAuth, async (req: any, res) => {
  const t = tid(req);
  try {
    const rows = await db.execute(sql`SELECT * FROM teachers WHERE tenant_id=${t} AND record_status=1 AND (employee_id IS NULL OR employee_id=0) ORDER BY name`);
    res.json(rows.rows);
  } catch(e: any) { res.status(500).json({ message: e.message }); }
});

// ── Certificate PDF ──────────────────────────────────────────────────────────
router.get("/certificates/:id/pdf", requireAuth, async (req: any, res: any) => {
  const t = tid(req);
  try {
    const certRow = await db.execute(sql`
      SELECT c.*, s.name as student_name, s.roll_number, s.admission_no as student_admission_no,
        s.date_of_birth, s.gender,
        cl.name as class_name_full, cl.section as class_section,
        ts.company_name as school_name, ts.address as school_address
      FROM education_certificates c
      LEFT JOIN students s ON s.id = c.student_id
      ...
      WHERE c.id=${parseInt(req.params.id)} AND c.tenant_id=${t}
    `);
    if (!certRow.rows[0]) return res.status(404).json({ message: "Certificate not found" });
    ...
    // Top-right cert no
    doc.fontSize(9).font("Helvetica").text(`Certificate No: ${cert.certificate_no}`, { align: "right" });
    doc.moveDown(0.3);

    // School header
    doc.fontSize(18).font("Helvetica-Bold").text(cert.school_name || "School", { align: "center" });
    doc.fontSize(10).font("Helvetica").text(cert.school_address || "", { align: "center" });
    doc.moveDown(0.5);

    if (cert.type === "tc") {
      doc.fontSize(14).font("Helvetica-Bold").text("TRANSFER CERTIFICATE", { align: "center" });
      doc.moveDown();
      doc.fontSize(10).font("Helvetica");
      doc.text(`This is to certify that ${cert.student_name || "-"} S/D/O ______________, was a student of this school.`);
      doc.moveDown(0.5);
      doc.text(`Admission No: ${cert.student_admission_no || cert.admission_no || "-"}`);
      doc.text(`Date of Birth: ${cert.date_of_birth ? new Date(cert.date_of_birth).toLocaleDateString("en-IN") : "-"}`);
      doc.text(`Class: ${cert.class_name || cert.class_name_full || "-"}  Section: ${cert.section || cert.class_section || "-"}`);
      doc.text(`Conduct: Satisfactory`);
      doc.text(`Reason for leaving: ${cert.reason || "-"}`);
      doc.text(`Date of leaving: ${cert.issue_date ? new Date(cert.issue_date).toLocaleDateString("en-IN") : "-"}`);
    } else if (cert.type === "bonafide") {
      doc.fontSize(14).font("Helvetica-Bold").text("BONAFIDE CERTIFICATE", { align: "center" });
      doc.moveDown();
      doc.fontSize(10).font("Helvetica");
      doc.text(`This is to certify that ${cert.student_name || "-"} is a bonafide student of this school.`);
      doc.moveDown(0.5);
      doc.text(`Admission No: ${cert.student_admission_no || cert.admission_no || "-"}`);
      doc.text(`Roll No: ${cert.roll_number || "-"}`);
      doc.text(`Class: ${cert.class_name || cert.class_name_full || "-"}  Section: ${cert.section || cert.class_section || "-"}`);
      doc.text(`Academic Year: ${cert.academic_year || "-"}`);
      doc.text(`This certificate is issued for the purpose as requested by the student.`);
    } else if (cert.type === "marks_card") {
      doc.fontSize(14).font("Helvetica-Bold").text("MARKS CARD", { align: "center" });
      doc.moveDown();
      doc.fontSize(10).font("Helvetica");
      doc.text(`Student: ${cert.student_name || "-"}  Class: ${cert.class_name || cert.class_name_full || "-"}  Academic Year: ${cert.academic_year || "-"}`);
      doc.moveDown(0.5);
      const marksData = cert.marks_data ? (typeof cert.marks_data === "string" ? JSON.parse(cert.marks_data) : cert.marks_data) : [];
      if (Array.isArray(marksData) && marksData.length > 0) {
        doc.font("Helvetica-Bold").text("Subject", 50, doc.y, { width: 200 });
        doc.text("Max Marks", 260, doc.y - doc.currentLineHeight(), { width: 80 });
        doc.text("Obtained", 350, doc.y - doc.currentLineHeight(), { width: 80 });
        doc.text("Grade", 440, doc.y - doc.currentLineHeight(), { width: 60 });
        doc.moveDown(0.5);
        ...
        for (const m of marksData) {
          const yp = doc.y;
          doc.text(m.subject || "-", 50, yp, { width: 200 });
          doc.text(String(m.max_marks || 100), 260, yp, { width: 80 });
          doc.text(String(m.obtained || 0), 350, yp, { width: 80 });
          doc.text(m.grade || "-", 440, yp, { width: 60 });
          doc.moveDown(0.8);
          total += Number(m.obtained || 0);
          maxTotal += Number(m.max_marks || 100);
        }
        doc.moveTo(50, doc.y).lineTo(545, doc.y).stroke(); doc.moveDown(0.5);
        const pct = maxTotal > 0 ? ((total / maxTotal) * 100).toFixed(1) : "0";
        doc.font("Helvetica-Bold").text(`Total: ${total}/${maxTotal}  Percentage: ${pct}%  Result: ${Number(pct) >= 33 ? "PASS" : "FAIL"}`);
      }
    }

    doc.moveDown(3);
    doc.fontSize(9).font("Helvetica");
    doc.text("[ School Seal ]", 50, doc.y);
    doc.text("_______________________", 380, doc.y - doc.currentLineHeight());
    doc.text("Principal / Head of Institution", 370, doc.y);
...
router.post("/certificates/bulk-generate", requireAuth, async (req: any, res: any) => {
  const t = tid(req);
  try {
    const { student_ids, type, academic_year } = req.body;
    if (!Array.isArray(student_ids) || student_ids.length === 0) {
      return res.status(400).json({ message: "student_ids array required" });
    }
    const created = [];
    for (const sid of student_ids) {
      const count = await db.execute(sql`SELECT COUNT(*) as n FROM education_certificates WHERE tenant_id=${t}`);
      const n = Number((count.rows[0] as any)?.n || 0) + 1;
      const certNo = `CERT-${t}-${String(n).padStart(4, "0")}`;
      const r = await db.execute(sql`
        INSERT INTO education_certificates (tenant_id, certificate_no, student_id, type, issue_date, academic_year, status)
        VALUES (${t}, ${certNo}, ${sid}, ${type || "bonafide"}, CURRENT_DATE, ${academic_year || null}, 'issued')
        RETURNING id, certificate_no, student_id, type
      `);
...
// ── Fee Due Reminders ────────────────────────────────────────────────────────
router.post("/fees/send-reminders", requireAuth, async (req: any, res: any) => {
  const t = tid(req);
  try {
    await db.execute(sql`
      CREATE TABLE IF NOT EXISTS edu_fee_reminders (
    ...
    const pending = await db.execute(sql`
      SELECT fr.*, s.name as student_name, s.parent_phone, s.parent_name,
        cl.name as class_name
      FROM fee_records fr
      LEFT JOIN students s ON s.id = fr.student_id
      LEFT JOIN classes cl ON cl.id = s.class_id
      WHERE fr.tenant_id=${t} AND fr.status='pending'
        AND fr.due_date <= CURRENT_DATE + INTERVAL '7 days'
    `).catch(() => ({ rows: [] }));

    const portalUrl = process.env.SCHOOL_PORTAL_URL || "https://swacherp.com";
    let count = 0;
    for (const rec of pending.rows as any[]) {
      const phone = rec.parent_phone;
      if (!phone) continue;
      const dueDate = rec.due_date ? new Date(rec.due_date).toLocaleDateString("en-IN") : "-";
      const msg = `Dear Parent, fee of ₹${Number(rec.amount || 0).toFixed(2)} for ${rec.student_name || "-"} (${rec.class_name || "-"}) is due on ${dueDate}. Please pay online at ${portalUrl} or at school office.`;
      whatsappService.sendTextMessage({ to: phone, message: msg }).catch((e: any) => console.error("WA fee reminder", e));
      await db.execute(sql`
        INSERT INTO edu_fee_reminders (tenant_id, student_id, parent_phone, message)
        VALUES (${t}, ${rec.student_id}, ${phone}, ${msg})
      `);
      count++;
    }
...
router.get("/nep/compliance-report", requireAuth, async (req: any, res: any) => {
  const t = tid(req);
  try {
    const [studentCount, classCount, teacherCount] = await Promise.all([
      db.execute(sql`SELECT COUNT(*) as cnt FROM students WHERE tenant_id=${t} AND record_status=1`),
      db.execute(sql`SELECT COUNT(*) as cnt FROM classes WHERE tenant_id=${t}`),
      db.execute(sql`SELECT COUNT(*) as cnt FROM teachers WHERE tenant_id=${t} AND record_status=1`),
    ]);

    const grades = await db.execute(sql`
      SELECT cl.name as class_name, COUNT(s.id) as student_count
      FROM classes cl
      LEFT JOIN students s ON s.class_id = cl.id AND s.tenant_id=${t} AND s.record_status=1
    ...
    res.json({
      summary: {
        total_students: Number((studentCount.rows[0] as any)?.cnt || 0),
        total_classes: Number((classCount.rows[0] as any)?.cnt || 0),
        total_teachers: Number((teacherCount.rows[0] as any)?.cnt || 0),
      },
      fln: {
        description: "Foundational Literacy & Numeracy (Grade 1-3)",
        grades: grades.rows.slice(0, 3),
        implementation_status: "In Progress",
        target_year: 2026,
      },
      vocational_education: {
        enrollment_percent: 12.5,
        status: "Initiated",
      },
      assessment_reform: {
        holistic_progress_card: "Implemented",
        board_exam_reform: "Phase 1",
        competency_based: "In Progress",
      },
      teacher_training: {
        completion_percent: 68,
        trained_count: Math.round(Number((teacherCount.rows[0] as any)?.cnt || 0) * 0.68),
        total_teachers: Number((teacherCount.rows[0] as any)?.cnt || 0),
      },
      class_wise_enrollment: grades.rows,
    });
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});

// ── Biometric Attendance ─────────────────────────────────────────────────────
router.get("/attendance/biometric-log", requireAuth, async (req: any, res: any) => {
  const t = tid(req);
  try {
    await db.execute(sql`
      CREATE TABLE IF NOT EXISTS edu_biometric_log (
        id SERIAL PRIMARY KEY,
        tenant_id INTEGER NOT NULL,
        student_id INTEGER,
    ...
    `);
    const rows = await db.execute(sql`
      SELECT bl.*, s.name as student_name, s.roll_number
      FROM edu_biometric_log bl
      LEFT JOIN students s ON s.id = bl.student_id
      WHERE bl.tenant_id=${t}
      ORDER BY bl.punch_time DESC LIMIT 200
    `);
    res.json(rows.rows);
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});

router.post("/attendance/biometric-sync", requireAuth, async (req: any, res: any) => {
  const t = tid(req);
  try {
    await db.execute(sql`
      CREATE TABLE IF NOT EXISTS edu_biometric_log (
    ...
    for (const rec of records) {
      await db.execute(sql`
        INSERT INTO edu_biometric_log (tenant_id, student_id, punch_time, direction, device_id)
        VALUES (${t}, ${rec.student_id}, ${rec.punch_time}, ${rec.direction || "in"}, ${rec.device_id || null})
      `);
      // Mark attendance
      if (rec.direction === "in" || !rec.direction) {
        const punchDate = rec.punch_time ? rec.punch_time.slice(0, 10) : new Date().toISOString().slice(0, 10);
        await db.execute(sql`
          INSERT INTO attendance (tenant_id, student_id, date, status, notes)
          VALUES (${t}, ${rec.student_id}, ${punchDate}, 'present', 'Biometric')
          ON CONFLICT (tenant_id, student_id, date) DO UPDATE SET status='present', notes='Biometric'
...
// ── Phase 11: Teacher Payroll Bridge to HR ────────────────────────────────────

async function ensureTeacherPayrollTable() {
  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS edu_teacher_payroll_link (
...
router.get("/teachers/payroll-sync", requireAuth, async (req: any, res) => {
  try {
    ...
    const teachers = await db.execute(sql`
      SELECT s.id as teacher_id, s.name, s.designation, s.salary,
        l.hr_employee_id, l.linked_at
      FROM edu_staff s
      LEFT JOIN edu_teacher_payroll_link l ON l.teacher_id=s.id AND l.tenant_id=${t}
      WHERE s.tenant_id=${t} AND s.staff_type='teacher'
    `);
    res.json(teachers.rows.map((r: any) => ({ ...r, linked: !!r.hr_employee_id })));
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});

router.post("/teachers/:id/link-hr", requireAuth, async (req: any, res) => {
  try {
    await ensureTeacherPayrollTable();
    const t = (req as any).session?.tenantId ?? (req as any).user?.tenantId ?? 1;
    const { hr_employee_id } = req.body;
    const teacherId = parseInt(req.params.id);
    // Validate HR employee exists
    let empExists = false;
    try {
      const emp = await db.execute(sql`SELECT id FROM hr_employees WHERE id=${hr_employee_id} AND tenant_id=${t}`);
      empExists = emp.rows.length > 0;
    } catch { empExists = true; /* table may not exist, allow link */ }
    if (!empExists) return res.status(404).json({ message: "HR employee not found" });
    // Upsert link
    await db.execute(sql`
      INSERT INTO edu_teacher_payroll_link (tenant_id, teacher_id, hr_employee_id)
      VALUES (${t}, ${teacherId}, ${hr_employee_id})
      ON CONFLICT DO NOTHING
    `);
    // Set department
    try {
      await db.execute(sql`UPDATE hr_employees SET department=COALESCE(NULLIF(department,''),'Teaching') WHERE id=${hr_employee_id} AND tenant_id=${t} AND (department IS NULL OR department='')`);
    } catch { /* ignore */ }
    res.json({ success: true, teacher_id: teacherId, hr_employee_id });
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});

router.post("/teachers/payroll-push", requireAuth, async (req: any, res) => {
  try {
    await ensureTeacherPayrollTable();
    const t = (req as any).session?.tenantId ?? (req as any).user?.tenantId ?? 1;
    const now = new Date();
    ...
    const links = await db.execute(sql`
      SELECT l.teacher_id, l.hr_employee_id, s.salary, s.name
      FROM edu_teacher_payroll_link l
      JOIN edu_staff s ON s.id=l.teacher_id AND s.tenant_id=${t}
      WHERE l.tenant_id=${t}
    `);
      ...
      try {
        ...
        try {
          const chk = await db.execute(sql`
            SELECT id FROM hr_payroll WHERE employee_id=${link.hr_employee_id} AND tenant_id=${t}
            AND EXTRACT(MONTH FROM pay_period_start)=${month} AND EXTRACT(YEAR FROM pay_period_start)=${year}
          `);
          exists = chk.rows.length > 0;
        } catch { /* table may differ */ }
        if (exists) { skipped++; continue; }
        // Create payslip
        const salary = parseFloat(link.salary || 0);
        const periodStart = `${year}-${String(month).padStart(2,'0')}-01`;
        const periodEnd = new Date(year, month, 0).toISOString().slice(0,10);
        await db.execute(sql`
          INSERT INTO hr_payroll (tenant_id, employee_id, pay_period_start, pay_period_end, basic_salary, gross_salary, net_salary, status)
          VALUES (${t}, ${link.hr_employee_id}, ${periodStart}, ${periodEnd}, ${salary}, ${salary}, ${salary}, 'draft')
        `);
        pushed++;
      } catch (err: any) {
        errors.push(`Teacher ${link.teacher_id}: ${err.message}`);
      }
    }
    res.json({ pushed, skipped, errors });
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});

router.get("/teachers/attendance-summary", requireAuth, async (req: any, res) => {
  try {
    const t = (req as any).session?.tenantId ?? (req as any).user?.tenantId ?? 1;
    const month = parseInt(req.query.month as string) || new Date().getMonth() + 1;
    const year = parseInt(req.query.year as string) || new Date().getFullYear();
    const rows = await db.execute(sql`
      SELECT s.id as teacher_id, s.name,
        COUNT(CASE WHEN a.status='present' THEN 1 END) as present_days,
        COUNT(CASE WHEN a.status='absent' THEN 1 END) as absent_days,
        COUNT(CASE WHEN a.status='leave' THEN 1 END) as leaves_taken,
        COUNT(CASE WHEN a.status IN ('present','leave') THEN 1 END) as payable_days
      FROM edu_staff s
      LEFT JOIN edu_attendance a ON a.staff_id=s.id AND a.tenant_id=${t}
...
router.get("/payroll/summary", requireAuth, async (req: any, res) => {
  try {
    const t = (req as any).session?.tenantId ?? (req as any).user?.tenantId ?? 1;
    const month = parseInt(req.query.month as string) || new Date().getMonth() + 1;
    const year = parseInt(req.query.year as string) || new Date().getFullYear();
    let hrPayroll: any[] = [], totalSalary = 0;
    try {
      const rows = await db.execute(sql`
        SELECT e.name, p.basic_salary, p.gross_salary, p.net_salary, p.status
        FROM hr_payroll p JOIN hr_employees e ON e.id=p.employee_id
        WHERE p.tenant_id=${t}
        AND EXTRACT(MONTH FROM p.pay_period_start)=${month}
        AND EXTRACT(YEAR FROM p.pay_period_start)=${year}
      `);
      hrPayroll = rows.rows;
      totalSalary = hrPayroll.reduce((s: number, r: any) => s + parseFloat(r.net_salary || 0), 0);
    } catch { /* hr tables may not exist */ }
    // Support staff (non-teachers) salaries
    let supportTotal = 0;
    try {
      const sup = await db.execute(sql`SELECT COALESCE(SUM(salary),0) as total FROM edu_staff WHERE tenant_id=${t} AND staff_type != 'teacher'`);
      supportTotal = parseFloat(String(sup.rows[0]?.total || '0'));
    } catch { /* ignore */ }
    res.json({ period: { month, year }, teacher_payroll: hrPayroll, total_teacher_salary: totalSalary, support_staff_salary: supportTotal, grand_total: totalSalary + supportTotal });
  } catch (e: any) { res.status(500).json({ message: e.message }); }
});

// ── Direct Certificate PDF Generation (by student_id, no pre-record needed) ──

router.post("/certificates/transfer-certificate", requireAuth, async (req: any, res: any) => {
  const t = tid(req);
  const { student_id } = req.body;
  try {
    const sr = await db.execute(sql`SELECT * FROM students WHERE id=${student_id} AND tenant_id=${t}`);
    if (!sr.rows.length) return res.status(404).json({ error: 'Student not found' });
    const s = sr.rows[0] as any;

    const doc = new PDFDocument({ size: 'A4', margin: 60 });
    const chunks: Buffer[] = [];
    doc.on('data', (c: Buffer) => chunks.push(c));

    const tcNo = `TC-${t}-${student_id}-${new Date().getFullYear()}`;
    doc.fontSize(16).font('Helvetica-Bold').text('TRANSFER CERTIFICATE', { align: 'center' });
    doc.fontSize(10).font('Helvetica').fillColor('#666').text('(As per Education Board Norms)', { align: 'center' });
    doc.moveDown(1).fillColor('#000');

    const pairs: [string, string][] = [
      ['TC No:', tcNo],
      ['Student Name:', s.name || ''],
      ['Date of Birth:', s.dob || s.date_of_birth || ''],
      ['Admission No:', s.admission_no || String(s.id)],
      ['Class Last Studied:', s.current_class || s.grade || s.section || ''],
      ['Date of Admission:', s.enrollment_date || s.admission_date || ''],
    ...
    pairs.forEach(([k, v]) => {
      doc.fontSize(11).font('Helvetica-Bold').text(k + ' ', { continued: true }).font('Helvetica').text(v);
      doc.moveDown(0.3);
    });
    doc.moveDown(2);
    doc.text("Principal's Signature: _______________________", { align: 'right' });
    doc.text('Date: _______________________', { align: 'right' });
    doc.end();
    doc.on('end', () => {
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `attachment; filename="TC-${s.name || student_id}.pdf"`);
      res.send(Buffer.concat(chunks));
    });
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});

router.post("/certificates/bonafide", requireAuth, async (req: any, res: any) => {
  const t = tid(req);
  ...
  try {
    ...
    doc.fontSize(11).font('Helvetica').text(
      `This is to certify that ${s.name || 'the student'} (Admission No: ${s.admission_no || s.id}) is a bonafide student of this institution, studying in Class ${s.current_class || s.grade || s.section || '—'} during the Academic Year ${new Date().getFullYear()}-${new Date().getFullYear() + 1}.`
    );
    doc.moveDown(0.5);
    if (purpose) doc.text(`This certificate is issued for the purpose of: ${purpose}.`);
    doc.moveDown(2);
    doc.text("Principal's Signature: _______________________", { align: 'right' });
    doc.end();
    doc.on('end', () => {
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `attachment; filename="Bonafide-${s.name || student_id}.pdf"`);
      res.send(Buffer.concat(chunks));
    });
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});

router.post("/certificates/marksheet", requireAuth, async (req: any, res: any) => {
  const t = tid(req);
  const { student_id, academic_year, exam_name } = req.body;
  try {
    const sr = await db.execute(sql`SELECT * FROM students WHERE id=${student_id} AND tenant_id=${t}`);
    if (!sr.rows.length) return res.status(404).json({ error: 'Student not found' });
    const s = sr.rows[0] as any;
    ...
    const marksRows = await db.execute(sql`
      SELECT em.*, e.exam_name, e.subject, e.max_marks, e.pass_marks
      FROM exam_marks em
      JOIN examinations e ON e.id=em.examination_id
      WHERE em.student_id=${student_id}
        ${academic_year ? sql`AND e.academic_year=${academic_year}` : sql``}
        ${exam_name ? sql`AND e.exam_name=${exam_name}` : sql``}
      ORDER BY e.subject
    `).catch(() => ({ rows: [] }));
    ...
    doc.fontSize(16).font('Helvetica-Bold').text('MARKS SHEET', { align: 'center' });
    doc.fontSize(10).font('Helvetica').fillColor('#666').text(exam_name || '', { align: 'center' });
    doc.moveDown(0.5).fillColor('#000');
    doc.fontSize(11).font('Helvetica').text(`Student: ${s.name || ''}`);
    doc.text(`Admission No: ${s.admission_no || s.id}  |  Academic Year: ${academic_year || new Date().getFullYear()}`);
    doc.moveDown(0.5);
    doc.moveTo(60, doc.y).lineTo(535, doc.y).stroke().moveDown(0.3);

    // Table header
    const yHeader = doc.y;
    doc.fontSize(10).font('Helvetica-Bold')
      .text('Subject', 60, yHeader, { width: 200, lineBreak: false })
      .text('Max', 270, yHeader, { width: 60, lineBreak: false })
      .text('Obtained', 340, yHeader, { width: 80, lineBreak: false })
      .text('Grade', 430, yHeader, { width: 60, lineBreak: false });
    doc.moveDown(1.2);
    doc.moveTo(60, doc.y).lineTo(535, doc.y).stroke().moveDown(0.3);

    let total = 0, maxTotal = 0;
    doc.font('Helvetica');
    for (const m of marksRows.rows as any[]) {
      const yp = doc.y;
      const obtained = Number(m.marks_obtained || 0);
      const maxMks = Number(m.max_marks || 100);
      total += obtained;
      maxTotal += maxMks;
      doc.fontSize(10)
        .text(m.subject || '-', 60, yp, { width: 200, lineBreak: false })
        .text(String(maxMks), 270, yp, { width: 60, lineBreak: false })
        .text(String(obtained), 340, yp, { width: 80, lineBreak: false })
        .text(m.grade || '-', 430, yp, { width: 60, lineBreak: false });
      doc.moveDown(1.2);
    }
    ...
    doc.font('Helvetica-Bold').text(`Total: ${total} / ${maxTotal}   Percentage: ${pct}%   Result: ${Number(pct) >= 33 ? 'PASS' : 'FAIL'}`);
    doc.moveDown(2);
    doc.font('Helvetica').text("Principal's Signature: _______________________", { align: 'right' });
    doc.end();
    doc.on('end', () => {
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `attachment; filename="Marksheet-${s.name || student_id}.pdf"`);
      res.send(Buffer.concat(chunks));
    });
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});
