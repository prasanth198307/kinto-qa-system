...
<body>
  ...
  // HPCL Invoice Migration - Preview December invoices that need updating
  app.get('/api/admin/preview-hpcl-invoice-migration', requireRole('admin'), async (req: any, res: Response) => {
    try {
      const hpclCorporate = {
        vendorName: 'VISAKH RETAIL RO Petronilayam, HPCL',
        address: 'Opp AU IN Gate, China Waltair, Visakhapatnam',
      ...
      // Get migrated vendor IDs (vendors with ship_to_name set)
      const migratedVendorIds = await db.execute(sql`
        SELECT v.id
        FROM vendors v
        JOIN vendor_vendor_types vvt ON v.id = vvt.vendor_id
        WHERE vvt.vendor_type_id = ${hpPaniType[0].id}
      ...
      const vendorIds = (migratedVendorIds.rows as any[]).map(v => v.id);
      ...
      if (vendorIds.length === 0) {
        return res.json({
          ...
          alreadyMigrated: 0,
          invoices: [],
          message: "No migrated HPCL vendors found. Run vendor migration first."
        });
      }

      // Find December 2024 invoices for these vendors that don't have HPCL corporate as buyer
      const decemberInvoices = await db.execute(sql`
        SELECT i.id, i.invoice_number, i.invoice_date, i.vendor_id,
               i.buyer_name, i.buyer_address, i.buyer_gstin,
               i.ship_to_name, i.ship_to_address,
               i.grand_total, i.status
        FROM invoices i
        WHERE i.vendor_id = ANY(${vendorIds})
          AND i.invoice_date >= '2024-12-01'
          AND i.record_status = 1
          AND i.buyer_name != ${hpclCorporate.vendorName}
        ORDER BY i.invoice_date DESC
      `);

      const invoices = decemberInvoices.rows as any[];
      ...
      res.json({
        totalInvoices: invoices.length + parseInt((alreadyMigrated.rows[0] as any).count),
        toMigrate: invoices.length,
        alreadyMigrated: parseInt((alreadyMigrated.rows[0] as any).count),
        hpclCorporate,
        invoices: invoices.map(inv => ({
          id: inv.id,
          invoiceNumber: inv.invoice_number,
          invoiceDate: inv.invoice_date,
          currentBuyer: inv.buyer_name,
          currentAddress: inv.buyer_address,
          currentGst: inv.buyer_gstin,
          hasShipTo: !!inv.ship_to_name,
          grandTotal: inv.grand_total,
          status: inv.status
        }))
      });
    } catch (error: any) {
      console.error('[HPCL Invoice Migration Preview] Error:', error);
      res.status(500).json({ message: error.message || 'Failed to preview invoice migration' });
    }
  });

  // HPCL Invoice Migration - Execute the update
  app.post('/api/admin/migrate-hpcl-invoices', requireRole('admin'), async (req: any, res: Response) => {
    try {
      const hpclCorporate = {
        vendorName: 'VISAKH RETAIL RO Petronilayam, HPCL',
        address: 'Opp AU IN Gate, China Waltair, Visakhapatnam',
      ...
      if (vendorIds.length === 0) {
        return res.status(404).json({ message: "No migrated HPCL vendors found. Run vendor migration first." });
      }
      ...
      await db.transaction(async (tx) => {
        // Get invoices to migrate
        const invoicesToMigrate = await tx.execute(sql`
          SELECT i.id, i.invoice_number, i.buyer_name, i.buyer_address, i.buyer_city,
                 i.buyer_state, i.buyer_pincode, i.buyer_gstin
          FROM invoices i
          WHERE i.vendor_id = ANY(${vendorIds})
            AND i.invoice_date >= '2024-12-01'
            AND i.record_status = 1
        ...
        for (const inv of invoicesToMigrate.rows as any[]) {
          // Update invoice: move buyer to ship-to, set HPCL as buyer
          await tx.execute(sql`
            UPDATE invoices SET
              ship_to_name = ${inv.buyer_name},
              ship_to_address = ${inv.buyer_address},
              ship_to_city = ${inv.buyer_city},
              ship_to_state = ${inv.buyer_state},
              ship_to_pincode = ${inv.buyer_pincode},
              ship_to_gstin = ${inv.buyer_gstin},
              buyer_name = ${hpclCorporate.vendorName},
              buyer_address = ${hpclCorporate.address},
              buyer_city = ${hpclCorporate.city},
              buyer_state = ${hpclCorporate.state},
              buyer_pincode = ${hpclCorporate.pincode},
              buyer_gstin = ${hpclCorporate.gstNumber}
            WHERE id = ${inv.id}
          `);

          migrationLog.push({
            invoiceNumber: inv.invoice_number,
            originalBuyer: inv.buyer_name,
            status: 'migrated',
            newShipTo: inv.buyer_name
          });
          migratedCount++;
        }
      });
      ...
      console.error('[HPCL Invoice Migration] Error:', error);
      res.status(500).json({ message: error.message || 'Failed to migrate invoices' });
    }
  });
