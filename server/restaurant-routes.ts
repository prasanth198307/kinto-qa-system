...
router.get("/wastage/summary", requireAuth, async (req, res) => {
  try {
    const result = await db.execute(sql`
      SELECT reason, COUNT(*)::int as count, COALESCE(SUM(quantity),0)::numeric as total_qty, COALESCE(SUM(total_cost),0)::numeric as total_cost
...
// ── Stock log (inventory movement history) ────────────────────────────────────
router.get("/stock/log", requireAuth, async (req: any, res: any) => {
  const t = tid(req);
  try {
    const rows = await db.execute(sql`
      SELECT * FROM restaurant_stock_log WHERE tenant_id=${t} ORDER BY created_at DESC LIMIT 200`);
    res.json(rows.rows || []);
  } catch { res.json([]); }
});

router.post("/stock/deduct", requireAuth, async (req: any, res: any) => {
  const t = tid(req);
  const { outlet_id, ingredient_name, quantity, unit, reason } = req.body;
  try {
    await db.execute(sql`
      INSERT INTO restaurant_stock_log (tenant_id, outlet_id, ingredient_name, quantity, unit, reason, action, created_at)
      VALUES (${t}, ${outlet_id||null}, ${ingredient_name}, ${quantity}, ${unit||'kg'}, ${reason||'manual'}, 'deduct', NOW())`);
    res.json({ success: true });
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});
...
router.get("/staff/schedules", requireAuth, async (req: any, res: any) => {
  ...
  try {
    const rows = await db.execute(sql`
      SELECT * FROM restaurant_staff_schedules
      WHERE tenant_id=${t}
        AND (${from as string} IS NULL OR shift_date >= ${from as string || '2000-01-01'}::date)
        AND (${to as string} IS NULL OR shift_date <= ${to as string || '2099-01-01'}::date)
...
router.post("/staff/schedules", requireAuth, async (req: any, res: any) => {
  const t = tid(req);
  const { employee_id, staff_name, staff_role, outlet_id, shift_date, shift_start, shift_end, notes } = req.body;
  try {
    const r = await db.execute(sql`
      INSERT INTO restaurant_staff_schedules (tenant_id, employee_id, staff_name, staff_role, outlet_id, shift_date, shift_start, shift_end, notes)
      VALUES (${t}, ${employee_id||null}, ${staff_name||null}, ${staff_role||null}, ${outlet_id||null}, ${shift_date}, ${shift_start||null}, ${shift_end||null}, ${notes||null})
      RETURNING id`);
  ...
  try {
    const rows = await db.execute(sql`
      SELECT
        ...
        COUNT(CASE WHEN a.status='absent' THEN 1 END)::int as days_absent,
        COUNT(CASE WHEN a.status='late' THEN 1 END)::int as days_late
      FROM hr_employees e
      LEFT JOIN hr_attendance a ON a.employee_id=e.id AND TO_CHAR(a.attendance_date,'YYYY-MM')=${(month as string) || new Date().toISOString().slice(0,7)}
...
router.get("/staff/waiter-performance", requireAuth, async (req: any, res: any) => {
  ...
  try {
    const rows = await db.execute(sql`
      SELECT waiter_name, waiter_employee_id,
        COUNT(*)::int as orders_handled,
      ...
      GROUP BY waiter_name, waiter_employee_id ORDER BY total_sales DESC LIMIT 20`);
    res.json(rows.rows || []);
  } catch { res.json([]); }
});

// ── Loyalty config & transactions ─────────────────────────────────────────────
router.get("/loyalty/config", requireAuth, async (req: any, res: any) => {
  const t = tid(req);
  try {
    const rows = await db.execute(sql`SELECT * FROM restaurant_loyalty_config WHERE tenant_id=${t} LIMIT 1`);
    res.json(rows.rows[0] || { points_per_rupee: 1, rupees_per_point: 0.5, min_redeem_points: 100, max_redeem_pct: 50, expiry_days: 365, is_active: true });
  } catch { res.json({ points_per_rupee: 1, rupees_per_point: 0.5, min_redeem_points: 100, max_redeem_pct: 50, expiry_days: 365, is_active: true }); }
});

router.put("/loyalty/config", requireAuth, async (req: any, res: any) => {
  const t = tid(req);
  const { points_per_rupee, rupees_per_point, min_redeem_points, max_redeem_pct, expiry_days, is_active } = req.body;
  try {
    await db.execute(sql`
      INSERT INTO restaurant_loyalty_config (tenant_id, points_per_rupee, rupees_per_point, min_redeem_points, max_redeem_pct, expiry_days, is_active)
      VALUES (${t},${points_per_rupee||1},${rupees_per_point||0.5},${min_redeem_points||100},${max_redeem_pct||50},${expiry_days||365},${is_active!==false})
      ON CONFLICT (tenant_id) DO UPDATE SET points_per_rupee=${points_per_rupee||1}, rupees_per_point=${rupees_per_point||0.5}, min_redeem_points=${min_redeem_points||100}, max_redeem_pct=${max_redeem_pct||50}, expiry_days=${expiry_days||365}, is_active=${is_active!==false}, updated_at=NOW()`);
    res.json({ success: true });
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});

router.post("/loyalty/earn", requireAuth, async (req: any, res: any) => {
  const t = tid(req);
  const { customer_id, points, order_id, order_amount } = req.body;
  try {
    await db.execute(sql`INSERT INTO restaurant_loyalty_transactions (tenant_id,customer_id,transaction_type,points,order_id,order_amount) VALUES (${t},${customer_id},'earn',${points},${order_id||null},${order_amount||null})`);
    await db.execute(sql`UPDATE restaurant_customers SET loyalty_points=COALESCE(loyalty_points,0)+${points} WHERE id=${customer_id} AND tenant_id=${t}`);
    res.json({ success: true });
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});

router.post("/loyalty/redeem", requireAuth, async (req: any, res: any) => {
  const t = tid(req);
  const { customer_id, points } = req.body;
  try {
    await db.execute(sql`INSERT INTO restaurant_loyalty_transactions (tenant_id,customer_id,transaction_type,points) VALUES (${t},${customer_id},'redeem',${-Math.abs(points)})`);
    await db.execute(sql`UPDATE restaurant_customers SET loyalty_points=GREATEST(0,COALESCE(loyalty_points,0)-${points}) WHERE id=${customer_id} AND tenant_id=${t}`);
    res.json({ success: true });
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});

// ── Delivery orders ───────────────────────────────────────────────────────────
router.get("/delivery-orders", requireAuth, async (req: any, res: any) => {
  const t = tid(req);
  const { status } = req.query;
  try {
    const rows = status
      ? await db.execute(sql`SELECT * FROM restaurant_delivery_orders WHERE tenant_id=${t} AND status=${status as string} ORDER BY created_at DESC`)
      : await db.execute(sql`SELECT * FROM restaurant_delivery_orders WHERE tenant_id=${t} ORDER BY created_at DESC LIMIT 100`);
    res.json(rows.rows || []);
  } catch { res.json([]); }
});

router.post("/delivery-orders", requireAuth, async (req: any, res: any) => {
  const t = tid(req);
  const { customer_name, customer_phone, delivery_address, items_json, subtotal, delivery_fee, discount, total, payment_mode, notes } = req.body;
  try {
    const r = await db.execute(sql`
      INSERT INTO restaurant_delivery_orders (tenant_id, customer_name, customer_phone, delivery_address, items_json, subtotal, delivery_fee, discount, total, payment_mode, notes, status)
      VALUES (${t},${customer_name||null},${customer_phone||null},${delivery_address||null},${JSON.stringify(items_json||[])}::jsonb,${subtotal||0},${delivery_fee||0},${discount||0},${total||0},${payment_mode||'cash'},${notes||null},'pending')
      RETURNING id`);
    res.json({ success: true, id: r.rows[0]?.id });
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});

router.put("/delivery-orders/:id", requireAuth, async (req: any, res: any) => {
  const t = tid(req);
  const { status, assigned_rider, estimated_delivery } = req.body;
  try {
    await db.execute(sql`
      UPDATE restaurant_delivery_orders SET status=${status||'pending'}, assigned_rider=${assigned_rider||null}, estimated_delivery=${estimated_delivery||null},
