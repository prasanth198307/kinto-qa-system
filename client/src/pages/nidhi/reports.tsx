...
const REPORT_TYPES = ["member-summary","deposit-report","loan-portfolio","emi-collection","overdue-report","balance-sheet"];

export default function NidhiReportsPage() {
  const [type, setType] = useState("member-summary");
  const [from, setFrom] = useState("");
  const [to, setTo] = useState("");
  const [enabled, setEnabled] = useState(false);

  const { data = [], isFetching } = useQuery({
    queryKey: ["/api/nidhi/reports", type, from, to],
    queryFn: () => api("GET", `/api/nidhi/reports/${type}?from=${from}&to=${to}`),
    enabled,
  });
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-bold">Nidhi Reports</h1>
      <Card>
