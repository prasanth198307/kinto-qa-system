...
function CropCyclesTab() {
  ...
  const { data: farms = [] } = useQuery<any[]>({ queryKey: ["/api/agriculture/farms"] });
  const { data: farmers = [] } = useQuery<any[]>({ queryKey: ["/api/agriculture/farmers"] });
  const { data: inputs = [] } = useQuery<any[]>({ queryKey: ["/api/agriculture/crop-inputs", showInputs?.id], enabled: !!showInputs, queryFn: () => fetch(`/api/agriculture/crop-inputs?crop_cycle_id=${showInputs.id}`, { credentials: "include" }).then(r => r.json()) });
