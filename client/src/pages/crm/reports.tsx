...
const REPORT_TYPES = ["rep-performance","pipeline-summary","revenue-forecast","lead-source-roi","activity-summary"];

export default function CRMReportsPage() {
  const [type, setType] = useState("rep-performance");
  ...
  const { data = [], isFetching } = useQuery({
    queryKey: ["/api/crm/reports", type, from, to, rep],
    queryFn: () => api("GET", `/api/crm/reports/${type}?from=${from}&to=${to}&rep=${rep}`),
    enabled,
  });
      ...
      <Card>
        <CardContent className="pt-4">
          <div className="flex gap-3 flex-wrap">
            ...
            <Input value={rep} onChange={e => setRep(e.target.value)} className="w-40" placeholder="Rep filter" />
          ...
          {isFetching ? <p className="text-muted-foreground">Loading...</p> : (
            <Table>
              ...
              <TableBody>
                ...
                {data.length === 0 && <TableRow><TableCell colSpan={Math.max(cols.length,1)} className="text-center text-muted-foreground">Generate a report above</TableCell></TableRow>}
              </TableBody>
            </Table>
          )}
        </CardContent>
