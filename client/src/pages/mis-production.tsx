...
import { AlertTriangle, Download, Info, CheckSquare, Square, BarChart2, Calendar } from "lucide-react";
...
export default function MISProduction() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'sku-monthly'>('overview');
  const [skuYear, setSkuYear] = useState(String(new Date().getFullYear()));

  const { data, isLoading } = useQuery<ProductionData>({ queryKey: ['/api/mis/production-analytics', { period }] });
  const { data: skuData, isLoading: isSkuLoading } = useQuery<SkuMonthlyData>({
    queryKey: ['/api/mis/production-sku-monthly', { year: skuYear }],
    enabled: activeTab === 'sku-monthly',
  });
  ...
  const handleExportSkuMonthly = async () => {
    if (!skuData) return;
    setIsExporting(true);
    try {
      const months = skuData.months;
      const monthLabels = months.map(m => MONTH_LABELS[m.slice(5, 7)] || m.slice(5, 7));
      const header = ['Product', 'Product Code', ...monthLabels, 'Total'];
      const rows = skuData.data.map(p => [
        p.productName,
        p.productCode,
        ...months.map(m => p.monthly[m]?.produced || 0),
        p.total,
      ]);
      const totalRow = ['TOTAL', '', ...months.map(m => skuData.monthlyTotals[m] || 0), skuData.grandTotal];
      await exportToExcel({
        filename: `production-sku-monthly-${skuYear}.xlsx`,
        sheets: [{ name: `SKU Monthly ${skuYear}`, data: [header, ...rows, totalRow] }],
      });
    } finally { setIsExporting(false); }
  };
  ...
  const yearOptions = [currentYear - 2, currentYear - 1, currentYear, currentYear + 1].map(String);
    ...
    <div className="p-4 md:p-6 space-y-6" data-testid="mis-production-page">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Efficiency, yield, and variance analysis</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {activeTab === 'sku-monthly' && (
            <>
              <Button variant="outline" size="sm" onClick={handleExportSkuMonthly} disabled={isExporting || isSkuLoading || !skuData} data-testid="button-export-sku-excel">
                <Download className="w-4 h-4 mr-2" />{isExporting ? 'Exporting...' : 'Export Excel'}
              </Button>
              <Select value={skuYear} onValueChange={setSkuYear}>
                <SelectTrigger className="w-[120px]" data-testid="select-sku-year"><SelectValue /></SelectTrigger>
                <SelectContent>
      ...
      <div className="flex rounded-md border w-fit">
        ...
        <Button
          variant={activeTab === 'sku-monthly' ? 'default' : 'ghost'}
          size="sm"
          onClick={() => setActiveTab('sku-monthly')}
          className="rounded-l-none"
          data-testid="button-tab-sku-monthly"
        >
          <Calendar className="w-4 h-4 mr-2" />
          Month-wise SKU Report
        </Button>
      </div>
        ...
        <>
          {isLoading ? (
            ...
            <>
              ...
              <Card>
                <CardHeader><CardTitle className="text-base">Production by Product</CardTitle><CardDescription>Top products by output volume</CardDescription></CardHeader>
                <CardContent>
                  <Table>
                    <TableHeader>
                      <TableRow>
              ...
              <Card>
                <CardHeader><CardTitle className="text-base">Daily Production Trend</CardTitle><CardDescription>Day-by-day production output</CardDescription></CardHeader>
                <CardContent>
      ...
      {/* Month-wise SKU Report Tab */}
      {activeTab === 'sku-monthly' && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Month-wise Production per SKU — {skuYear}</CardTitle>
            <CardDescription>Total produced quantity for each product broken down by month</CardDescription>
          </CardHeader>
          <CardContent>
            {isSkuLoading ? (
            ...
            ) : !skuData || skuData.data.length === 0 ? (
              <div className="text-center py-12 text-muted-foreground">
                <Calendar className="w-10 h-10 mx-auto mb-3 opacity-30" />
                <p className="font-medium">No production data for {skuYear}</p>
                <p className="text-sm mt-1">Production entries logged in {skuYear} will appear here.</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead className="min-w-[160px] sticky left-0 bg-background z-10">Product</TableHead>
                      {skuData.months.map(m => (
                        <TableHead key={m} className="text-right min-w-[70px] text-xs">
                          {MONTH_LABELS[m.slice(5, 7)]}
                        </TableHead>
                      ))}
                      <TableHead className="text-right font-bold min-w-[80px]">Total</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {skuData.data.map((product) => (
                      <TableRow key={product.productId} data-testid={`row-sku-${product.productId}`}>
                        ...
                        <TableCell className="text-right font-bold text-sm">{fmt(product.total)}</TableCell>
                      </TableRow>
                    ))}
                    {/* Monthly totals row */}
                    <TableRow className="border-t-2 bg-muted/30 font-semibold">
                      <TableCell className="sticky left-0 bg-muted/30 z-10 text-sm font-bold">Total</TableCell>
                      {skuData.months.map(m => (
                        <TableCell key={m} className="text-right text-sm font-bold">
                          {skuData.monthlyTotals[m] ? fmt(skuData.monthlyTotals[m]) : <span className="text-muted-foreground text-xs font-normal">—</span>}
                        </TableCell>
                      ))}
                      <TableCell className="text-right font-bold text-sm text-primary">{fmt(skuData.grandTotal)}</TableCell>
                    </TableRow>
                  </TableBody>
                </Table>
              </div>
