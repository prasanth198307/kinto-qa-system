...
const MODE_CONFIG: Record<Mode, {
  ...
  "retail": {
    ...
    previewCols: ["Barcode", "Product Name", "SKU", "HSN", "GST%", "MRP", "Selling ₹", "UOM"],
    fields: [
      { col: "Barcode / EAN",           req: false, note: "13-digit EAN or internal barcode. Duplicate = update existing item." },
      { col: "Product Name",            req: true,  note: "Full name as on packet." },
      { col: "SKU Code",                req: false, note: "Internal stock-keeping code." },
      { col: "Category",                req: false, note: "Aisle / shelf category." },
      { col: "HSN Code",                req: true,  note: "6-digit HSN for GST billing." },
      { col: "GST %",                   req: true,  note: "0 / 5 / 12 / 18." },
      { col: "MRP (₹)",                 req: true,  note: "Max retail price printed on packet." },
      { col: "Purchase Rate (₹)",       req: false, note: "Cost from distributor / supplier." },
      { col: "Selling Price (₹)",       req: true,  note: "POS counter price ≤ MRP." },
      { col: "UOM",                     req: true,  note: "PCS / KG / LTR / PKT." },
      { col: "Sold By (unit/weight)",   req: false, note: "unit = fixed price; weight = per kg via scale." },
      { col: "Reorder Level",           req: false, note: "Low-stock alert threshold." },
      { col: "Item Type (goods/service)", req: false, note: "goods (default)." },
    ],
  },
};
...
export default function InventoryBulkImportPage() {
  ...
  const renderRow = (r: any, col: string, i: number) => {
    ...
    // retail
    const vals: Record<string, any> = {
      "Barcode": r.barcode || "—", "Product Name": r.product_name,
      "SKU": r.sku_code || "—", "HSN": r.hsn_code,
