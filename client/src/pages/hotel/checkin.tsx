            ...
            <Card key={r.id}>
              <CardContent className="pt-4 flex items-start justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500">{r.adults ?? 1} adults{r.children ? ` · ${r.children} children` : ""} · {r.source} · {sym}{Number(r.total_amount ?? 0).toLocaleString("en-IN")}</p>
                </div>
            ...
            <Card key={r.id}>
              <CardContent className="pt-4 flex items-start justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500">Total {sym}{Number(r.total_amount ?? 0).toLocaleString("en-IN")} · Advance {sym}{Number(r.advance_paid ?? 0).toLocaleString("en-IN")} · Balance {sym}{Number((r.total_amount ?? 0) - (r.advance_paid ?? 0)).toLocaleString("en-IN")}</p>
                </div>
                <div className="flex gap-2">
