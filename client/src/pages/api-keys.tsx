                    ...
                    <div className="mt-3 pl-7 space-y-3">
                      ...
                      <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                        {(() => {
                          ...
                          const filtered = catalogue.filter(api =>
                            !q || api.label.toLowerCase().includes(q) || api.path.toLowerCase().includes(q) || api.method.toLowerCase().includes(q)
                          );
                          if (filtered.length === 0) {
                            return <p className="text-xs text-muted-foreground italic py-2">No APIs match "{scopeSearch}"</p>;
                          }
                      ...
                      {selectedScopes.length > 0 && (
                        <p className="text-xs text-muted-foreground" onClick={e => e.stopPropagation()}>
                          {selectedScopes.length} API{selectedScopes.length !== 1 ? 's' : ''} selected
                        </p>
                      )}
                    </div>
                  )}
