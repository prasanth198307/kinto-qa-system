...
export const tenants = pgTable("tenants", {
  ...
  status: varchar("status", { length: 50 }).default('trial'),
  ...
  taxRegime: varchar("tax_regime", { length: 20 }).default("gst"),
...
export const insertUserSchema = createInsertSchema(users, {
  ...
  mobileNumber: z.string().regex(/^[0-9]{10}$/, "Mobile number must be 10 digits").optional().nullable(),
...
export const vendors = pgTable("vendors", {
  ...
  paymentMode: varchar("payment_mode", { length: 20 }).default('bill_to_bill'), // 'bill_to_bill' or 'cod'
  isActive: varchar("is_active").default('true'),
...
export const finishedGoods = pgTable("finished_goods", {
  ...
  repackingDate: timestamp("repacking_date", { mode: 'string' }), // Date when repacking was completed
  recordStatus: integer("record_status").default(1).notNull(),
  createdBy: varchar("created_by").references(() => users.id),
  createdAt: timestamp("created_at", { mode: 'string' }).defaultNow(),
  updatedAt: timestamp("updated_at", { mode: 'string' }).defaultNow(),
...
export const chartOfAccounts = pgTable("chart_of_accounts", {
  ...
  description: text("description"),
  isActive: integer("is_active").default(1).notNull(),
  isSystemAccount: integer("is_system_account").default(0).notNull(), // 1 = seeded, cannot delete
  recordStatus: integer("record_status").default(1).notNull(),
  createdAt: timestamp("created_at", { mode: 'string' }).defaultNow(),
  updatedAt: timestamp("updated_at", { mode: 'string' }).defaultNow(),
  tenantId: integer("tenant_id").default(1),
...
// Journal Entries - Header record for each journal
export const journalEntries = pgTable("journal_entries", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  journalNumber: varchar("journal_number", { length: 100 }).unique().notNull(), // JRN-YYYYMMDD-{seq} or AUTO-YYYYMMDD-{seq}
  journalDate: date("journal_date").notNull(),
  
  // Source linking for auto-generated entries
  sourceType: varchar("source_type", { length: 50 }), // invoice, payment, customer_advance, advance_application, credit_note, debit_note, vendor_debit_note, vdn_adjustment, expense, write_off, manual
  sourceId: varchar("source_id"), // ID of the source record
  
  description: text("description").notNull(),
  status: varchar("status", { length: 20 }).default('posted').notNull(), // draft, posted, reversed
  isAutoGenerated: integer("is_auto_generated").default(0).notNull(), // 1 = auto-generated from transaction
  
  // Totals (for quick validation — sum of debits should equal sum of credits)
  totalDebit: integer("total_debit").default(0).notNull(), // in paise
  totalCredit: integer("total_credit").default(0).notNull(), // in paise
  
  // Metadata
  createdBy: varchar("created_by").references(() => users.id),
  reversedAt: timestamp("reversed_at", { mode: 'string' }),
  reversalJournalId: varchar("reversal_journal_id"), // Points to the reversing entry
  notes: text("notes"),
  ...
  index("journal_date_idx").on(table.journalDate),
  index("journal_source_idx").on(table.sourceType, table.sourceId),
]);
...
// Journal Lines - Debit/Credit line items for each journal entry
export const journalLines = pgTable("journal_lines", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  journalId: varchar("journal_id").references(() => journalEntries.id).notNull(),
  accountId: varchar("account_id").references(() => chartOfAccounts.id).notNull(),
  
  debit: integer("debit").default(0).notNull(), // in paise
  credit: integer("credit").default(0).notNull(), // in paise
  
  // Party reference (for sub-ledger tracking)
  partyType: varchar("party_type", { length: 20 }), // vendor, employee, bank
  ...
  index("jl_journal_idx").on(table.journalId),
