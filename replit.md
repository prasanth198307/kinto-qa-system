...
- **Invoice-First Gatepass Flow:** Invoices must be created first (independently), then selected when creating a gatepass. When an invoice is selected in the gatepass form, all items (products, quantities, UOM) are automatically populated from the invoice and displayed as read-only - users cannot manually add, remove, or edit items. The "Add Item" and "Remove Item" buttons are hidden when an invoice is selected. This ensures complete data consistency between invoices and gatepasses. For exceptional cases without an invoice, users can create a manual gatepass by leaving the invoice field empty, which allows full CRUD on items. Once an invoice is linked to an active gatepass, it cannot be reused for another gatepass. However, when a gatepass is deleted (soft deleted), the linked invoice becomes available for reuse.
