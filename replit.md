...
- **Advance Payments (Prepayments) System:** Payment Management includes an "Advance Payments" tab to record customer payments not linked to any specific invoice (type='prepayment', numbered PRE-YYYYMMDD-NNN). These differ from Customer Advances (security deposits, type='security_deposit', numbered ADV-YYYYMMDD-NNN). Available advance balances appear in the Pending Payments dashboard as a green "Avail. Advance" column and in the header aggregate. Advances can be applied to specific invoices from Payment Management using the existing advance-apply endpoint. DB column: `customer_advances.advance_type`.
- **Per-Item Discount System:** Both Invoices and Sales Orders support per-line-item discounts with two modes: `%` (percentage of gross line value) and `₹` (flat rupee amount per case × qty). GST is always calculated on the post-discount taxable value. Discount fields: `discount` (integer, value × 100) and `discountMode` (`%` or `₹`) on both `invoice_items` and `sales_order_items`. When creating an invoice from a Sales Order, discount values are automatically carried across. Migration: `db_scripts/2026-04-03_sales_order_items_discount.sql`.
