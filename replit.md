...
- **Invoice-First Gatepass Flow:** Invoices must be created first (independently), then selected when creating a gatepass. Once an invoice is linked to a gatepass, it cannot be reused for another gatepass. This ensures proper one-to-one relationship and prevents invoice duplication. Backend validation enforces this business rule.
