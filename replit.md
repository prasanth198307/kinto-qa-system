...
## Recent Changes
- **2025-11-13**: Fixed Product Master Multi-Tab Interface (TC 17.1) - Resolved critical tab switching bug after 13 debugging iterations. Root cause: useEffect with unstable dependency array `[item, open, existingBom, replace]` was resetting `activeTab` to "info" on every render, preventing conditional rendering from working. Solution: Split into TWO separate useEffects - (1) Form reset effect with stable deps `[open, item?.id]` that only runs when dialog opens or different item selected, (2) BOM hydration effect with `[existingBom]` that runs when BOM data loads asynchronously. Implementation: Replaced Radix UI Tabs with custom div-based tabs using `onClick={() => setActiveTab('value')}` and `{activeTab === 'info' && (<div>...)}` conditional rendering. This production-ready solution ensures tabs only reset when dialog opens, not on every re-render, while BOM data loads correctly on edit.
