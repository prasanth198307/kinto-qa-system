...
- **Invoice-First Gatepass Flow:** Invoices must be created first (independently), then selected when creating a gatepass. The items section is **only displayed when an invoice is selected**. When an invoice is selected, all items (products, quantities, UOM) are automatically populated from the invoice and displayed as read-only - users cannot manually add, remove, or edit items. If no invoice is selected, the items section remains hidden, enforcing the invoice-first workflow. This ensures complete data consistency between invoices and gatepasses. Once an invoice is linked to an active gatepass, it cannot be reused for another gatepass. However, when a gatepass is deleted (soft deleted), the linked invoice becomes available for reuse.
