...
- **Daily Cash Register:** Petty cash tracking per salesperson with Excel import capability. Features include daily balance tracking (opening, deposits, received cash, expenses, transfers, closing), item detail parsing from imported data, reconciliation workflow (pending → confirmed), conversion of cash register expenses to formal expense vouchers, and duplicate prevention through voucher linking.
