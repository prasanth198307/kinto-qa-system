...
## Recent Changes
- **2025-11-13**: Fixed Product Master Multi-Tab Interface & BOM Save (TC 17.1) - Resolved TWO critical issues after extensive debugging: (1) **Tab Switching Bug** - Root cause: useEffect with unstable dependency array `[item, open, existingBom, replace]` was resetting `activeTab` to "info" on every render. Solution: Split into TWO separate useEffects with stable dependencies `[open, item?.id]` and `[existingBom]` for form reset and BOM hydration respectively. (2) **BOM Save Failure** - Root causes: Variable scoping issue where productId wasn't properly captured, and stale field array data causing undefined rawMaterialId/quantityRequired in BOM payload. Solution: Fixed productId scope, added guard clause, and built BOM payload from submitted form values with proper type coercion. Implementation: Replaced Radix UI Tabs with custom div-based tabs using `onClick` handlers and conditional rendering. Both tabs and BOM save now work correctly in production.
