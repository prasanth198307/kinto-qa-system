...
- **Payment Evidence System:** Two-stream payment architecture where Sale Report's "Amount Received" column (VY- payments) is the immutable authoritative ledger, and Payments.xlsx records are stored as linked payment_evidence children for audit trail. Evidence matching uses required vendor match, exact/partial amount match, date proximity (±7 days), and reference similarity. Evidence records include match_status (matched/orphan/manual), match_confidence percentage, and source row data. Payment Management UI shows expandable VY- payment rows with linked evidence records. Enriched Payment Report API (`/api/invoice-payments/with-evidence`) provides batched evidence summaries (count, total amount, references, payment modes) with pagination and filtering for reporting without affecting financial totals.
