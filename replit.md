...
- **ESS (Employee Self-Service) Portal:** Separate authenticated portal at `/ess` (public login) and `/ess/portal` (dashboard). Uses its own session keys (`essEmployeeId`, `essTenantId`) separate from admin session. Login: company slug + emp_code + password. Tabs: Home (summary cards), Pay Slips (view/print), Attendance (monthly + self check-in/check-out), Leave (balance + apply), Tax Declaration (80C/80D/HRA/Other), Profile. Admin enables ESS for employees via "Set ESS Password" button (key icon) in employee list — calls `POST /api/ess/admin/set-password`. DB columns: `ess_password VARCHAR(200)` and `ess_enabled BOOLEAN` on `hr_employees`. Type-based navigation: Consultant gets Home+Payslips+Attendance+TaxDecl+Profile; Contract gets same + Leave; Intern gets same; Permanent gets all tabs.
- **ESS Self Attendance Marking:** Employees can check in/out from ESS attendance tab. DB: `check_in_time TIME`, `check_out_time TIME`, `working_hours NUMERIC(5,2)`, `marked_by VARCHAR(20)` added to `hr_attendance`. ESS endpoint: `POST /api/ess/attendance/mark` auto-detects check-in vs check-out. Working hours auto-calculated on checkout.
- **Leave Approval Workflow:** `hr_leave_applications` has `status` (pending/approved/rejected/cancelled), `approved_by`, `approver_comment`, `action_at`. ESS submits with `status=pending`. Admin approves/rejects in HR Leaves page (`hr-leaves.tsx`). On approval, leave balance is deducted automatically. WhatsApp notification sent to employee on action.
- **TDS Declaration Approval Workflow:** `hr_tds_declarations` has `status` (submitted/resubmitted/approved/rejected), `approved_by`, `approver_comment`, `approved_at`. ESS submission sets `status=submitted`; re-submission after approval sets `status=resubmitted` and clears approval fields. Admin reviews in TDS & Compliance page — Approve (tick) or Reject (X with comment) buttons per row. Pending alert shown at top. ESS shows colored status banners (green=approved, blue=pending, amber=resubmitted, red=rejected with HR comment).
...
- **Multi-Role Users:** Each user can be assigned multiple roles simultaneously. DB: `user_roles` junction table `(user_id, role_id, tenant_id)`. `users.role_id` kept as "primary role" for backward compat. Permission checks (`requireRole`, `hrPermissionMiddleware`, `/api/my-permissions`) union all roles — if ANY role grants access, the request is allowed. `GET/PUT /api/users/:id/roles` endpoints manage multi-role assignments. Frontend `AdminUserManagement.tsx` shows checkboxes for role selection. `usePermissions()` returns `roles: string[]` (all assigned role names).
