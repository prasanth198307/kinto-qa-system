...
- **Bug #10 - Raw Material Issuance Foreign Key Error:** Made `productId` optional in `rawMaterialIssuanceItems` schema (shared/schema.ts line 724). Removed `.notNull()` constraint to allow issuance without product selection, preventing foreign key constraint violations.
- **Bug #11 - Checklist Template Items Not Persisted:** Fixed data validation in AdminChecklistBuilder.tsx by sending `undefined` instead of empty strings for optional `verificationCriteria` field. Added comprehensive error handling and logging to storage.createChecklistTemplate() for better diagnostics.
- **Bug #12 - Reviewer Role Permissions:** Expanded reviewer permissions from 3 to 14 screens in database_scripts/02_seed_data.sql. Reviewer now has view access to dashboard, machines, machine_types, checklist_templates, checklists, spare_parts, purchase_orders, maintenance_plans, pm_templates, pm_execution, products, raw_materials, finished_goods, and reports (with edit permission only on checklists).
- **Bug #13 - Invoice Form Submission:** Added onError callbacks to InvoiceForm.tsx mutation to display backend validation errors in toast notifications. Removed TypeScript errors by eliminating references to non-existent product.hsnCode field.
- **Bug #14 - PM Maintenance Plan Creation:** Fixed 400 validation error by converting empty `machineId` strings to `undefined` in SchedulePMDialog.tsx. Prevents foreign key constraint violations when machine is not selected during PM plan creation.
- **Bug #15 - Checklist Assignment Error Handling:** Enhanced ManagerChecklistAssignment.tsx with improved error callbacks, form validation, and user-friendly error messages to properly display backend validation failures.
