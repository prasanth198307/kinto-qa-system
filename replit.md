...
- **Comprehensive Reporting:** Generates printable, branded reports, sales/overview dashboards, and unified operational/GST reports. Printable components include: Invoice, Gatepass, Credit Note, Raw Material Issuance, Purchase Order, Debit Note, PM Execution, Cash Register Voucher, Sales Return, Scrap Inventory, and Expense Voucher. All print components handle loading states to prevent printing incomplete documents.
...
- **Comprehensive Role Permissions Management:** Granular access control across 37+ system screens with metadata-driven UI. Backend `endpointToScreenKey` mapping (80+ entries) ensures API-level permission enforcement for custom roles. Frontend navigation filtering via `navItemToScreenKey` and `navItemToScreen` mappings. Includes `vendor_debit_notes` screen key for vendor debit note access control.
- **Raw Material & Product Master Systems:** Manages raw material definitions with conversion methods, loss percentages, and comprehensive product management with Bill of Materials (BOM). Raw material form includes Received Date field for ongoing inventory tracking.
- **BOM-Driven Production:** Intelligent material issuance based on BOMs and production entry with BOM variance analysis. BOM now references Material Types (not individual raw materials) via `materialTypeId`, enabling dynamic raw material selection during issuance based on available stock. Auto-populates raw material issuance form when product is selected, with automatic stock selection when single option available.
- **Multi-BOM Configuration System:** Products can have multiple BOM configurations (e.g., "Kinto 1L" with 21gm preform vs 19.2gm preform options). Each configuration defines different material requirements for the same product. During raw material issuance, users select which configuration to use. Configurations have names, descriptions, and a default flag. The system tracks which configuration was used for each issuance for variance analysis and costing.
- **FIFO Batch Allocation:** When issuing raw materials, the system automatically allocates from oldest batches first (based on receivedDate). Batch codes follow LOT-YYYYMMDD format. Multi-batch production scenarios show detailed allocation breakdown with batch codes, dates, and quantities per batch.
...
- **Customer Advances System:** Track and manage advance payments received from customers before invoicing. Features include advance number generation (ADV-YYYYMMDD-{seq}), multi-payment method support (Cash, Cheque, NEFT, RTGS, UPI, Card), status workflow (active → fully_used or cancelled), transaction-safe advance application to invoices with row-level locking, automatic invoice payment record creation, and balance tracking. Accessible via `/customer-advances` route with screen key `customer_advances`.
- **Vendor Debit Notes System:** Manual debit note creation against vendors for claims (defective goods, short receipts, quality rejections, price disputes). Features multi-item entries with GST breakdown, settlement tracking, status workflow (draft → issued → acknowledged → settled), and comprehensive list view with search/filter. Accessible to admin, manager, and AccountsManager roles.
...
- **Payment Evidence System:** Two-stream payment architecture where Sale Report's "Amount Received" column (VY- payments) is the immutable authoritative ledger, and Payments.xlsx records are stored as linked payment_evidence children for audit trail. Evidence matching uses required vendor match, exact/partial amount match, date proximity (±7 days), and reference similarity. Evidence records include match_status (matched/orphan/manual), match_confidence percentage, source row data, and direct invoice linking via invoice_id. Payment Management UI shows expandable VY- payment rows with linked evidence records. Invoice-level evidence API (`/api/invoice-evidence/:invoiceId`) aggregates all evidence for an invoice regardless of which payment it's linked to. Enriched Payment Report API (`/api/invoice-payments/with-evidence`) provides batched evidence summaries (count, total amount, references, payment modes) with pagination and filtering for reporting without affecting financial totals.
...
- **Inventory Management Logic:** Inventory deduction on gatepass creation. Raw Material inventory supports "Opening Stock Entry Only" and "Ongoing Inventory" modes. Finished goods require explicit quality approval. Invoice/gatepass cancellations trigger automatic inventory returns. **Logical Stock Reservation:** When creating invoices, available stock is calculated by deducting quantities already reserved in draft or ready-for-gatepass invoices, preventing overselling before physical dispatch.
