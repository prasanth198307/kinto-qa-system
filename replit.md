...
## Recent Changes
- **2025-11-13**: Finance & Sales Testing Complete - Fixed CRITICAL Invoice → Gatepass workflow bug: (1) **Invoice Status Update Bug** - Fixed gatepass creation setting invoice status back to "ready_for_gatepass" instead of "dispatched" (routes.ts line 2897). Bug was a copy-paste error. After fix: invoice status correctly transitions from "ready_for_gatepass" → "dispatched" on gatepass creation. (2) **Inventory Deduction Verified** - Confirmed finished goods inventory deduction logic works correctly (lines 2924-2926): deducts quantity on gatepass creation with row-level locking. (3) **Testing Summary**: Invoice creation ✅ (GST calculations correct), Invoice → Gatepass flow ✅ (status update + inventory deduction working), Gatepass detail page ⚠️ (404 - missing UI route, business logic works). Finance workflows production-ready with one UI gap.
- **2025-11-13**: TC 21 PRODUCTION-READY - Production Entry System Complete - Successfully resolved FIVE critical bugs enabling full Production Entry functionality: (1) **ProductionEntryForm API Request Bug** - Fixed parameter order in apiRequest call: `apiRequest('POST', '/api/production-entries', data)` (ProductionEntryForm.tsx line 88). (2) **Schema Null Validation Bug** - Fixed `rawMaterialIssuanceItemSchema` to accept null values for optional fields: added `z.null()` union and `.nullable()` (schema.ts lines 1079, 1085). (3) **Cache Invalidation Bug** - Added finished-goods cache invalidation with `exact: false` flag (ProductionEntryForm.tsx line 92). (4) **ISS-Number Display Bug** - Modified `getAllProductionEntries()` to LEFT JOIN with rawMaterialIssuance table and return issuanceNumber; updated frontend to display `{(entry as any).issuanceNumber || entry.issuanceId}` (storage.ts lines 1600-1623, production-entries.tsx line 131). (5) **CRITICAL Inventory Deduction Bug** - Implemented robust isOpeningStockOnly check supporting both integer (0/1) and boolean (false/true): `const isOngoingInventory = !material.isOpeningStockOnly || material.isOpeningStockOnly === 0` (routes.ts lines 1996-2024). **Inventory Logic**: Materials with isOpeningStockOnly=1/true skip deduction (Opening Stock Only mode), while isOpeningStockOnly=0/false/null deduct stock (Ongoing Inventory mode). **Architect Approved**: Both display and inventory fixes validated as production-ready. Production Entries table now shows ISS-numbers correctly, and stock deduction respects inventory mode settings with full audit trail logging.
- **2025-11-13**: TC 17.1 PRODUCTION-READY - Product Master Multi-Tab BOM Interface - Fixed THREE critical production bugs after comprehensive debugging: (1) **API Response Parsing** - Added `await response.json()` to parse apiRequest Response object (was returning undefined product IDs). (2) **BOM Payload Type Alignment** - Drizzle-Zod maps PostgreSQL `numeric` columns to `z.string()` for precision preservation (official behavior) - changed payload to send `String(quantityRequired)` instead of `Number()` to match schema contract. (3) **Storage Field Name Mismatch** - Fixed `getProductBom` to return schema-compliant field names `quantityRequired`/`uom` (was returning legacy `quantityPerProduct`/`unitOfMeasure` causing edit dialog to show quantity=0). Removed all debug logging for production security. All 4 TC 17.1 test scenarios passing: create with BOM, edit BOM hydration (15.75 displays correctly), validation enforcement, empty BOM acceptance. Implementation includes pre-save BOM validation (Step 0), atomic BOM transaction, partial success handling, and stable multi-tab interface.
