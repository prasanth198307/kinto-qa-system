...
- **Invoice & Gatepass Management:** Includes template management, enhanced invoice forms with print preview, and multi-screen printing options. **Smart Invoice Item Entry:** When selecting a product in invoice items, the system auto-fills base price, HSN code, and GST rate from Product Master while keeping all fields editable for custom pricing (discounts, special deals). This ensures pricing consistency while maintaining flexibility.
...
- **Production Entry System with BOM Variance Analysis:** Comprehensive production tracking system that records actual manufacturing output linked to raw material issuances by shift (A/B/General) with real-time variance analysis. Operators select an issuance, enter production date and shift, record produced and rejected quantities, and track empty bottle inventory (produced/used/pending). System auto-calculates derived units (produced quantity × usable derived units from Product Master) for quality metrics. Features include: Issuance selector with auto-populated product and planned output details, shift-based entry (A/B/General) with unique constraint preventing duplicate entries per issuance/date/shift, quantity inputs for produced/rejected/empty bottles, server-side derived units calculation for tamper-proof metrics, and collapsible BOM variance analysis table displaying Material, Type, UOM, Conversion Method, Expected Quantity (auto-calculated from actual output using BOM formulas), Issued Quantity (from issuance records), and Variance (issued - expected) with color-coded indicators (green/yellow/red). The variance analysis enables real-time material efficiency tracking by comparing planned BOM requirements against actual material consumption. **Automatic Inventory Integration:** When a production entry is created, the system automatically adds the produced quantity to Finished Goods inventory with auto-generated batch numbers (format: IssuanceNumber-Date-Shift), quality status set to 'pending' for inspection, and complete traceability linking back to the production entry. Database schema enforces unique composite constraint on (issuanceId, productionDate, shift), with 409 conflict handling for duplicate attempts. Accessible via Manager and Admin dashboards under Operations/Production Operations navigation section with list view, stats cards (Total Entries, Total Produced, Total Rejected), and full CRUD operations through modal dialogs.
...
- **Inventory Management Logic:** Inventory deduction occurs on gatepass creation (physical dispatch). Raw Material inventory supports dual stock management modes: Opening Stock Entry Only mode for setting baseline inventory with openingStock and openingDate, and Ongoing Inventory mode for dynamic tracking with receivedQuantity, returnedQuantity, and adjustments. Backend auto-calculates closingStock and closingStockUsable (closingStock × usableUnits from Type Master) for both modes, with zero-value and string-numeral normalization. **Finished Goods Quality Approval:** Finished goods are created with 'pending' quality status and require explicit approval or rejection by authorized personnel. The approval workflow includes one-click approve/reject buttons in the inventory management UI for pending items, automatic inspection timestamp recording, and visual status indicators (approved=green, pending=gray, rejected=red). When production entries automatically create finished goods inventory, items are marked as 'pending' to ensure quality verification before use in sales or dispatch operations. **Invoice & Gatepass Cancellation:** Invoices can only be cancelled if no gatepass has been created (no inventory deducted yet). Once a gatepass exists, the system blocks invoice deletion with an error message showing the gatepass number. When a gatepass is cancelled, the system automatically returns all dispatched quantities back to Finished Goods inventory, maintaining accurate stock levels. This ensures inventory integrity and prevents phantom deductions.
