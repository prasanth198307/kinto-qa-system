...
## Recent Changes
- **2025-11-13**: TC 17.1 COMPLETE - Product Master Multi-Tab BOM Interface (ARCHITECT APPROVED) - Resolved THREE critical issues through iterative debugging: (1) **Tab Switching** - Split useEffect into two separate effects with stable dependencies to prevent continuous tab resets while allowing async BOM loading. (2) **BOM Save Workflow** - Fixed productId scoping, added guard clause, and built proper BOM payload from submitted form values. (3) **Validation & Data Integrity** - Implemented pre-save BOM validation (Step 0) that blocks product creation if any BOM rows are incomplete, preventing orphaned products. Validation checks all rows for non-empty rawMaterialId and positive quantityRequired, throws descriptive error with row numbers if validation fails. Partial success handling keeps dialog open in update mode when BOM API fails after product save, enabling retry without duplicates. Implementation uses custom div-based tabs with onClick handlers for maximum reliability. All flows tested and production-ready.
