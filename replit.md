...
- **Sales Returns & Damage Handling System:** Manages post-delivery returns with a three-stage workflow (Pending → Received → Inspected). Includes quality segregation and inventory reconciliation, creating new Finished Goods inventory records with appropriate batch suffixes and quality statuses ('approved'/'rejected') based on inspection.
