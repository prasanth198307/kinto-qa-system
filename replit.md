...
- **Invoice-First Gatepass Flow:** Invoices must be created first (independently), then selected when creating a gatepass. Once an invoice is linked to an active gatepass, it cannot be reused for another gatepass. However, when a gatepass is deleted (soft deleted), the linked invoice becomes available for reuse. This ensures proper one-to-one relationship for active gatepasses while allowing invoice reuse after deletion. Backend validation enforces this business rule.
