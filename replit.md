...
## Recent Changes
- **2025-11-13**: TC 21 Production Entry Bug Fixes - Comprehensive debugging session resolved THREE critical bugs preventing Production Entry feature from working: (1) **ProductionEntryForm API Request Bug** - Fixed parameter order in apiRequest call: changed `apiRequest(url, {method, body})` → `apiRequest('POST', '/api/production-entries', data)` (line 88). (2) **Schema Null Validation Bug** - Fixed `rawMaterialIssuanceItemSchema` to accept null values for optional fields: added `z.null()` to suggestedQuantity union type (line 1079), added `.nullable()` to calculationBasis enum (line 1085), preventing 400 validation errors when client sends null. (3) **Cache Invalidation Bug** - Added finished-goods cache invalidation with `exact: false` flag to ProductionEntryForm onSuccess handler (line 92), ensuring all finished-goods queries (including parameterized) refresh when production entries create auto-finished-goods. **Testing Results**: Production entries create successfully ✅, finished goods auto-created ✅, UI refreshes immediately ✅. **Known Remaining Issues**: (a) Production Entries table displays issuance UUIDs instead of ISS-numbers (display bug - needs JOIN query fix), (b) Raw material stock not deducting on issuance creation (CRITICAL inventory bug - requires investigation of stock deduction workflow in routes.ts).
