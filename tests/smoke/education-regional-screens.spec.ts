/**
 * Education ERP — Multi-region screen smoke tests
 *
 * Covers 5 regional education_enterprise tenants:
 *   UAE  (9972 / qa_edu_ae_owner) — AED, VAT
 *   USA  (9973 / qa_edu_us_owner) — USD, Sales Tax
 *   EU   (9974 / qa_edu_eu_owner) — EUR, VAT
 *   SG   (9975 / qa_edu_sg_owner) — SGD, GST
 *   AU   (9976 / qa_edu_au_owner) — AUD, GST
 */
...
const REGIONS: RegionConfig[] = [
  { username: 'qa_edu_ae_owner', currency: 'AED', currencySymbol: 'د.إ', taxLabel: 'VAT', country: 'UAE'       },
  { username: 'qa_edu_us_owner', currency: 'USD', currencySymbol: '$',   taxLabel: 'Tax', country: 'USA'       },
  { username: 'qa_edu_eu_owner', currency: 'EUR', currencySymbol: '€',   taxLabel: 'VAT', country: 'Germany'   },
  { username: 'qa_edu_sg_owner', currency: 'SGD', currencySymbol: 'S$',  taxLabel: 'GST', country: 'Singapore' },
  { username: 'qa_edu_au_owner', currency: 'AUD', currencySymbol: 'A$',  taxLabel: 'GST', country: 'Australia' },
];
...
for (const region of REGIONS) {
  test.describe(`Education ${region.country} (${region.currency})`, () => {

    test(`all core education screens load without crash`, async ({ browser }) => {
...
test.describe('Multi-region consistency', () => {
  ...
  test('India tenant still shows INR', async ({ browser }) => {
    const { page, context } = await loginAs(browser, 'qa_edu_owner');
    try {
      const body = await assertNoCrash(page, '/education/fees');
      const hasINR = body.includes('₹') || body.includes('INR');
      expect(hasINR, 'India education fees page missing ₹/INR').toBeTruthy();
    } finally {
      await context.close();
    }
  });
  ...
  test('plan features API returns education vertical-specific nav items for all regions', async ({ browser }) => {
    for (const region of REGIONS) {
      const { page, context } = await loginAs(browser, region.username);
      try {
        ...
        expect(nav, `${region.country} missing education in nav`).toContain('education');
