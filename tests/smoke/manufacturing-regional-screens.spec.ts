/**
 * Manufacturing ERP — Multi-region screen smoke tests
 *
 * 5 regional manufacturing_enterprise tenants:
 *   UAE  (8222 / qa_mfg_ae_owner) — AED, VAT
 *   USA  (8223 / qa_mfg_us_owner) — USD, Sales Tax
 *   EU   (8224 / qa_mfg_eu_owner) — EUR, VAT, Germany
 *   SG   (8225 / qa_mfg_sg_owner) — SGD, GST
 *   AU   (8226 / qa_mfg_au_owner) — AUD, GST
 */
...
const REGIONS: RegionConfig[] = [
  { username: 'qa_mfg_ae_owner', currency: 'AED', currencySymbol: 'د.إ', taxLabel: 'VAT', country: 'UAE'       },
  { username: 'qa_mfg_us_owner', currency: 'USD', currencySymbol: '$',    taxLabel: 'Tax', country: 'USA'       },
  { username: 'qa_mfg_eu_owner', currency: 'EUR', currencySymbol: '€',    taxLabel: 'VAT', country: 'Germany'   },
  { username: 'qa_mfg_sg_owner', currency: 'SGD', currencySymbol: 'S$',   taxLabel: 'GST', country: 'Singapore' },
  { username: 'qa_mfg_au_owner', currency: 'AUD', currencySymbol: 'A$',   taxLabel: 'GST', country: 'Australia' },
];
...
for (const region of REGIONS) {
  test.describe(`Manufacturing ${region.country} (${region.currency})`, () => {

    test('all core manufacturing screens load without crash', async ({ browser }) => {
...
test.describe('Manufacturing Multi-region consistency', () => {
  ...
  test('India manufacturing tenant still shows INR after regional tenants exist', async ({ browser }) => {
    const { page, context } = await loginAs(browser, 'qa_mfg_owner');
    try {
      const body = await assertNoCrash(page, '/manufacturing/cost-sheets');
      const hasINR = body.includes('₹') || body.includes('INR');
      expect(hasINR, 'India manufacturing cost-sheets missing ₹/INR').toBeTruthy();
    } finally {
      await context.close();
    }
  });
  ...
  test('plan features API returns manufacturing module for all regional tenants', async ({ browser }) => {
    for (const region of REGIONS) {
      const { page, context } = await loginAs(browser, region.username);
      try {
        ...
        expect(nav, `${region.country} missing manufacturing in nav`).toContain('manufacturing');
  ...
  test('enterprise plan features include warehouses and fixed-assets for all regional tenants', async ({ browser }) => {
    for (const region of REGIONS) {
      const { page, context } = await loginAs(browser, region.username);
      try {
        const features = await page.evaluate(async () => {
