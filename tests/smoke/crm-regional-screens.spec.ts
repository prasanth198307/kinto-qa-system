/**
 * CRM ERP — Multi-region screen smoke tests
 *
 * Covers 5 regional crm_enterprise tenants:
 *   UAE  (9622 / qa_crm_ae_owner) — AED, VAT
 *   USA  (9623 / qa_crm_us_owner) — USD, Sales Tax
 *   EU   (9624 / qa_crm_eu_owner) — EUR, VAT
 *   SG   (9625 / qa_crm_sg_owner) — SGD, GST
 *   AU   (9626 / qa_crm_au_owner) — AUD, GST
 */
...
const REGIONS: RegionConfig[] = [
  { username: 'qa_crm_ae_owner', currency: 'AED', currencySymbol: 'د.إ', taxLabel: 'VAT', country: 'UAE'       },
  { username: 'qa_crm_us_owner', currency: 'USD', currencySymbol: '$',   taxLabel: 'Tax', country: 'USA'       },
  { username: 'qa_crm_eu_owner', currency: 'EUR', currencySymbol: '€',   taxLabel: 'VAT', country: 'Germany'   },
  { username: 'qa_crm_sg_owner', currency: 'SGD', currencySymbol: 'S$',  taxLabel: 'GST', country: 'Singapore' },
  { username: 'qa_crm_au_owner', currency: 'AUD', currencySymbol: 'A$',  taxLabel: 'GST', country: 'Australia' },
];
...
for (const region of REGIONS) {
  test.describe(`CRM ${region.country} (${region.currency})`, () => {

    test(`all core CRM screens load without crash`, async ({ browser }) => {
...
test.describe('Multi-region consistency', () => {
  ...
  test('India tenant still shows INR', async ({ browser }) => {
    ...
    try {
      const body = await assertNoCrash(page, '/crm');
      const hasINR = body.includes('₹') || body.includes('INR');
      expect(hasINR, 'India CRM page missing ₹/INR').toBeTruthy();
    } finally {
      await context.close();
    }
  });
  ...
  test('plan features API returns CRM vertical-specific nav items for all regions', async ({ browser }) => {
    for (const region of REGIONS) {
      const { page, context } = await loginAs(browser, region.username);
      try {
        ...
        expect(nav, `${region.country} missing crm in nav`).toContain('crm');
