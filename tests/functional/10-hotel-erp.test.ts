...
describe('Hotel ERP — 1. Room Setup', () => {
  ...
  it('POST /api/hotel/rooms creates a room', async () => {
    ...
    const body = await json<{ id: string; room_number: string }>(res);
    roomId = body.id as any;
...
describe('Hotel ERP — 3. Check-In', () => {
  it('POST /api/hotel/check-in checks guest in', async () => {
    ...
    const res = await api.post(`/api/hotel/reservations/${reservationId}/checkin`, {
      room_id: roomId ?? null,
    ...
    if (res.status === 403 || res.status === 404 || res.status === 422 || res.status === 500) return;
    const body = await json<{ id: string | number; folio_id?: string | number; status: string }>(res);
    bookingId = body.id as any;
    if (body.folio_id) folioId = body.folio_id as any;
    expect(body.status).toMatch(/checked_in|active|confirmed/);
  });

  it('GET /api/hotel/front-desk returns current occupancy', async () => {
    const res = await api.get('/api/hotel/front-desk');
    if (res.status === 404 || res.status === 403 || res.status === 500) return;
...
describe('Hotel ERP — 4. Folio (In-House Charges)', () => {
  it('GET /api/hotel/folio/:id returns guest folio', async () => {
    if (!folioId) return;
    const res = await api.get(`/api/hotel/folio/${folioId}`);
    if (res.status === 403 || res.status === 404 || res.status === 500) return;
    const folio = await json<{ id: string | number }>(res);
...
describe('Hotel ERP — 5. Check-Out & Billing', () => {
  it('POST /api/hotel/check-out generates final folio bill', async () => {
    if (!reservationId) return;
    const res = await api.post(`/api/hotel/reservations/${reservationId}/checkout`, {
      actual_check_out: TODAY,
      payment_method: 'card',
    });
    if (res.status === 403 || res.status === 404 || res.status === 422 || res.status === 500) return;
    const body = await json<{ total_amount?: number; status?: string }>(res);
...
describe('Hotel ERP — 8. Custom Roles & Permissions API', () => {
  it('GET /api/roles returns roles for hotel tenant', async () => {
  ...
  it('POST /api/roles creates a custom role for hotel tenant', async () => {
    const res = await api.post('/api/roles', {
      name: 'QA Concierge',
      description: 'Custom role for hotel concierge desk',
      permissions: ['hotel_read', 'hotel_write', 'guest_checkin'],
    });
    expect(res.status).not.toBe(404);
  });
});

describe('Hotel ERP — 9. Masters: Regions, Branches, Tax Config, Audit Log', () => {
...
describe('Hotel ERP — 10. Cross-Module Integration', () => {
  it('GET /api/accounting/journal-entries works alongside hotel (GL cross-module)', async () => {
    const res = await api.get('/api/accounting/journal-entries?limit=5');
    expect(res.status).not.toBe(404);
  });

  it('GET /api/hr/employees works alongside hotel (HR cross-module)', async () => {
    const res = await api.get('/api/hr/employees');
    expect(res.status).not.toBe(404);
  });

  it('GET /api/crm/contacts works alongside hotel (CRM cross-module)', async () => {
    const res = await api.get('/api/crm/contacts');
    expect(res.status).not.toBe(404);
  });

  it('GET /api/mis/summary works alongside hotel (MIS cross-module)', async () => {
    const res = await api.get('/api/mis/summary');
    expect(res.status).not.toBe(404);
  });

  it('GET /api/invoices works alongside hotel folio billing (AR cross-module)', async () => {
    const res = await api.get('/api/invoices?limit=5');
    expect(res.status).not.toBe(404);
  });

  it('GET /api/warehouses works alongside hotel linen/inventory (Warehouse cross-module)', async () => {
