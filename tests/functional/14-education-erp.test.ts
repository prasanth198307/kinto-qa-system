...
describe('Education ERP — 7. Custom Roles & Permissions API', () => {
  it('GET /api/roles returns roles for education tenant', async () => {
  ...
  it('POST /api/roles creates a custom role for education tenant', async () => {
    const res = await api.post('/api/roles', {
      name: 'QA Curriculum Lead',
      description: 'Custom role for curriculum management',
      permissions: ['education_read', 'education_write', 'timetable_manage'],
    });
    expect(res.status).not.toBe(404);
  });
});

describe('Education ERP — 8. Masters: Regions, Branches, Tax Config, Audit Log', () => {
...
describe('Education ERP — 9. Cross-Module Integration', () => {
  it('GET /api/accounting/journal-entries works alongside education (GL cross-module)', async () => {
    const res = await api.get('/api/accounting/journal-entries?limit=5');
    expect(res.status).not.toBe(404);
  });

  it('GET /api/hr/employees works alongside education (HR cross-module)', async () => {
    const res = await api.get('/api/hr/employees');
    expect(res.status).not.toBe(404);
  });

  it('GET /api/crm/contacts works alongside education (CRM cross-module)', async () => {
    const res = await api.get('/api/crm/contacts');
    expect(res.status).not.toBe(404);
  });

  it('GET /api/mis/summary works alongside education (MIS cross-module)', async () => {
    const res = await api.get('/api/mis/summary');
    expect(res.status).not.toBe(404);
  });

  it('GET /api/invoices works alongside education fee collection (AR cross-module)', async () => {
    const res = await api.get('/api/invoices?limit=5');
    expect(res.status).not.toBe(404);
  });

  it('GET /api/payroll works alongside education teacher payroll (Payroll cross-module)', async () => {
